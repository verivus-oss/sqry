...
fn staged_nodes(source: &str) -> Vec<(String, u32, u32)> {
    ...
    sqry_lang_haskell::HaskellGraphBuilder::default()
        .build_graph(&tree, source.as_bytes(), Path::new("P.hs"), &mut staging)
        .expect("build graph");
...
fn declaration_reports_its_real_line_not_line_one() {
    ...
    let hit = nodes
        .iter()
        .find(|(name, ..)| name.contains("Shape"))
        .unwrap_or_else(|| panic!("no node matching `Shape` in {nodes:?}"));

    assert_eq!(
        hit.1, 5,
        "`Shape` is declared on line 5, got line {} col {} (col {} would be its byte offset)",
        hit.1, hit.2, hit.2
    );
    assert_ne!(
        hit.1, 1,
