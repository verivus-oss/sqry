//! Malformed input tests for Haskell language plugin.
//!
...
fn test_deeply_nested_shallow() {
    let plugin = HaskellPlugin::default();
    let nested = MalformedInputBuilder::for_language("haskell").deeply_nested(depths::SHALLOW);
...
fn test_deeply_nested_medium() {
    let plugin = HaskellPlugin::default();
    let nested = MalformedInputBuilder::for_language("haskell").deeply_nested(depths::MEDIUM);
...
fn test_deeply_nested_deep() {
    let plugin = HaskellPlugin::default();
    let nested = MalformedInputBuilder::for_language("haskell").deeply_nested(depths::DEEP);
...
fn test_oversized_1mb() {
    let plugin = HaskellPlugin::default();
    let large = MalformedInputBuilder::for_language("haskell").oversized_1mb();

    // This is valid Haskell (repeated), so parse should succeed
...
fn test_oversized_10mb() {
    let plugin = HaskellPlugin::default();
    let large = MalformedInputBuilder::for_language("haskell").oversized_10mb();
...
fn test_random_bytes() {
    ...
    // Random bytes very unlikely to be valid Haskell, but tree-sitter is error-tolerant
    let _ = result;
}
...
// Relations tests removed - graph builder coverage lives in graph_builder_tests.
