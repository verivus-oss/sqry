...
import Data.Maybe hiding (fromJust)
    ...
    fn has_ffi_edge(
        ...
        staging.operations().iter().any(|op| {
            ...
            {
                // Extract canonical symbol from target node name for exact matching
                // Strip "ffi::{convention}::" prefix, split on whitespace, drop leading '&'
                *edge_conv == convention
                    && node_lookup.get(target).is_some_and(|(name, _)| {
                        if !name.starts_with(&ffi_prefix) {
                            return false;
                        }

                        // Extract symbol after prefix
                        let symbol = name
                            .strip_prefix(&ffi_prefix)
                            .unwrap_or("")
                            .split_whitespace()
                            .last()
                            .unwrap_or("")
                        ...
                        // Exact match on canonical symbol
...
foreign import ccall "sin" c_sin :: Double -> Double
        ...
        // Check that wrapper function has qualified name
        let string_map = build_string_map(&staging);
