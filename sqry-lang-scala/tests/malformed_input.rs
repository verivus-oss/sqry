//! Malformed input tests for Scala language plugin.
//!
...
fn test_deeply_nested_shallow() {
    let plugin = ScalaPlugin::default();
    let nested = MalformedInputBuilder::for_language("scala").deeply_nested(depths::SHALLOW);
...
fn test_deeply_nested_medium() {
    let plugin = ScalaPlugin::default();
    let nested = MalformedInputBuilder::for_language("scala").deeply_nested(depths::MEDIUM);
...
fn test_deeply_nested_deep() {
    let plugin = ScalaPlugin::default();
    let nested = MalformedInputBuilder::for_language("scala").deeply_nested(depths::DEEP);
...
fn test_oversized_1mb() {
    let plugin = ScalaPlugin::default();
    let large = MalformedInputBuilder::for_language("scala").oversized_1mb();

    // This is valid Scala (repeated), so parse should succeed
...
fn test_oversized_10mb() {
    let plugin = ScalaPlugin::default();
    let large = MalformedInputBuilder::for_language("scala").oversized_10mb();
...
fn test_random_bytes() {
    ...
    // Random bytes very unlikely to be valid Scala, but tree-sitter is error-tolerant
    let _ = result;
}
...
fn test_build_graph_on_malformed() {
    let plugin = ScalaPlugin::default();
    let malformed = MalformedInputBuilder::truncated_utf8();
    let path = Path::new("test.scala");

    if let Ok(tree) = plugin.parse_ast(&malformed) {
        let builder = plugin
            .graph_builder()
            .expect("Scala plugin should provide GraphBuilder");
