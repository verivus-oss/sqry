...
| **Versions** | `scripts/sync-versions.sh` (run with `--fix` if stale) |
...
**EdgeKind** (26 variants with metadata):
...
3. Architecture: unified graph since v2.0.0 (all 35 plugins, arena+CSR storage)
...
### Serialization
- `postcard` for graph index files (`.sqry/graph/snapshot.sqry`)
...
Submit reviews using your repository review workflow script and store outputs under `docs/reviews/<component>/<YYYY-MM-DD>/`.
...
See `docs/templates/DEVELOPMENT_PROCESS.md` for the full workflow.
...
|--------|---------------|
| Bug fix ≤50 LOC | `04_PROGRESS-_SLUG_.md` + `06_TEST_EXECUTION-_SLUG_.md` |
...
5. `05_TEST_PLAN-_SLUG_.md` - Verification
6. `06_TEST_EXECUTION-_SLUG_.md` - Results
...
## References
- **Review Workflow**: `docs/templates/DEVELOPMENT_PROCESS.md`
