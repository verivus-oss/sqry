//! Malformed input tests for HTML language plugin.
//!
...
fn test_deeply_nested_shallow() {
    let plugin = HtmlPlugin::default();
    let nested = MalformedInputBuilder::for_language("html").deeply_nested(depths::SHALLOW);
...
fn test_deeply_nested_medium() {
    let plugin = HtmlPlugin::default();
    let nested = MalformedInputBuilder::for_language("html").deeply_nested(depths::MEDIUM);
...
fn test_deeply_nested_deep() {
    let plugin = HtmlPlugin::default();
    let nested = MalformedInputBuilder::for_language("html").deeply_nested(depths::DEEP);
...
fn test_oversized_1mb() {
    let plugin = HtmlPlugin::default();
    let large = MalformedInputBuilder::for_language("html").oversized_1mb();

    // This is valid HTML (repeated), so parse should succeed
...
fn test_oversized_10mb() {
    let plugin = HtmlPlugin::default();
    let large = MalformedInputBuilder::for_language("html").oversized_10mb();
...
fn test_random_bytes() {
    ...
    // Random bytes very unlikely to be valid HTML, but tree-sitter is error-tolerant
    let _ = result;
}
...
// Note: HTML is a markup language without relations (imports/exports/calls)
