    ...
    match tag_name.as_deref() {
        Some("link") => {
            ...
            if let Some(href) = attributes.get("href")
                ...
                && !href.starts_with('#')
            {
                let import_id = helper.add_import(href, None);
        ...
        Some("img") => {
            ...
            {
                let asset_id = helper.add_variable(src, None);
        ...
        Some("a") => {
            ...
            {
                let link_id = helper.add_import(href, None);
        ...
        Some("iframe") => {
            ...
            {
                let frame_id = helper.add_import(src, None);
...
fn extract_script_resources(
    ...
    if let Some(src) = attributes.get("src")
        && !src.starts_with("data:")
    {
        let script_id = helper.add_import(src, None);
