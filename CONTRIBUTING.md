...
sqry is a semantic code search tool built in Rust that understands code structure through AST analysis. It supports 35 languages, provides CLI, LSP, and MCP interfaces, and uses a unified graph architecture for fast code analysis.

**Key stats**:
- 35 language plugins (24 with full relation support)
- 8,300+ tests passing across all platforms
- Native MCP server for AI assistants (Claude, Codex, Gemini, Cursor, Windsurf)
- LSP server for editors (VS Code, Neovim, Helix)
...
The repository uses a 3-phase clippy compliance workflow for new code: Phase 1 (errors), Phase 2 (warnings), Phase 3 (pedantic). See `AGENTS.md` for the full clippy process requirements.
...
| `sqry-lsp` | LSP server with 28+ handlers (hover, definition, references, call hierarchy) |
...
|-------------|----------------------|
| Bug fix (<50 LOC) | `04_PROGRESS` + `06_TEST_EXECUTION` entry |
...
For major features, create `docs/development/<feature-name>/` with:

1. `01_SPEC.md` - What and why (requirements, goals)
2. `02_DESIGN.md` - How (architecture, data structures)
3. `03_IMPLEMENTATION_PLAN.md` - Steps with acceptance criteria
4. `04_PROGRESS.md` - Real-time status tracking
5. `05_TEST_PLAN.md` - Verification strategy
6. `06_TEST_EXECUTION.md` - Actual test results
...
Key steps within the **Test** job: malformed input tests (FFI safety across 34 plugin suites), slop scan (AI code quality guardrails), graphsync RKG check.
...
Plugins emit nodes with `NodeKind` variants: `Function`, `Method`, `Class`, `Interface`, `Trait`, `Module`, `Variable`, `Constant`, `Type`, `Struct`, `Enum`, `EnumVariant`, `Macro`, `Import`, `Export`, `Component`, `Service`, `Resource`, `Endpoint`, `Test`, `Other`.
...
1. Use the plugin template in `docs/templates/PLUGIN_TEMPLATE-_SLUG_.md`
2. Study an existing plugin close to your target language (e.g., `sqry-lang-go` for a compiled language, `sqry-lang-python` for a dynamic language)
...
- [CODEX.md](CODEX.md) - Internal development guide for Codex CLI
- [GEMINI.md](GEMINI.md) - Internal development guide for Gemini CLI
- [AGENTS.md](AGENTS.md) - Multi-agent entrypoint and compatibility notes
