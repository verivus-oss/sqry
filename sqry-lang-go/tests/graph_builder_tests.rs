...
use sqry_test_support::graph_helpers::{
    assert_call_edge_has_span, assert_has_call_edge, find_call_edge,
};
use std::path::Path;
...
fn test_simple_function_calls() {
    ...
    assert_has_call_edge(&staging, "main.main", "main.helper");
    assert_has_call_edge(&staging, "main.helper", "fmt.Println");

    let helper_call = find_call_edge(&staging, "main.main", "main.helper")
        .expect("expected call edge main.main -> main.helper");
    assert_eq!(helper_call.argument_count, 0);
    assert!(!helper_call.is_async);

    let print_call = find_call_edge(&staging, "main.helper", "fmt.Println")
...
fn test_method_calls() {
    ...
    assert_has_call_edge(&staging, "main.main", "Increment");
    assert_has_call_edge(&staging, "main.main", "GetValue");

    let increment_call = find_call_edge(&staging, "main.main", "Increment")
...
fn test_package_qualified_calls() {
    ...
    assert_has_call_edge(&staging, "utils.ProcessString", "strings.ToUpper");
    assert_has_call_edge(&staging, "utils.ProcessString", "fmt.Println");
}
...
fn test_receiver_types() {
    ...
    assert_has_call_edge(&staging, "main.main", "Distance");
    assert_has_call_edge(&staging, "main.main", "Move");
}
...
fn test_interfaces() {
    ...
    assert_has_call_edge(&staging, "main.processData", "Read");
    assert_has_call_edge(&staging, "main.processData", "Write");
}
...
fn test_nodes_have_correct_language() {
    ...
    assert!(
        has_interned_string(&staging, "main.main"),
        "should intern main.main node name"
    );
    assert!(
        has_interned_string(&staging, "main.helper"),
...
fn test_edges_have_metadata() {
    ...
    let helper_call = find_call_edge(&staging, "main.main", "main.helper")
        .expect("expected call edge main.main -> main.helper");
    assert_eq!(helper_call.argument_count, 0);
    assert_call_edge_has_span(&staging, "main.main", "main.helper");
...
fn test_nested_functions() {
    ...
    assert_has_call_edge(&staging, "main.outer", "main.inner");
    assert_has_call_edge(&staging, "main.outer", "main.helper");
}
...
fn test_multiple_packages() {
    ...
    assert_has_call_edge(&staging, "config.Load", "os.Open");
    assert_has_call_edge(&staging, "config.Load", "io.ReadAll");
    assert_has_call_edge(&staging, "config.Load", "json.Unmarshal");
}
...
fn test_package_qualification() {
    ...
    assert!(
        has_interned_string(&staging, "utils.ProcessString"),
...
/// Helper to check if a string exists in interned strings
fn has_interned_string(staging: &StagingGraph, pattern: &str) -> bool {
...
";
    ...
    assert!(
        has_interned_string(&staging, "main.Child"),
        "Should have main.Child struct"
    );
    assert!(
        has_interned_string(&staging, "main.Base"),
