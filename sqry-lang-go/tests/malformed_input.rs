//! Malformed input tests for Go language plugin.
//!
...
fn test_deeply_nested_shallow() {
    let plugin = GoPlugin::default();
    let nested = MalformedInputBuilder::for_language("go").deeply_nested(depths::SHALLOW);
...
fn test_deeply_nested_medium() {
    let plugin = GoPlugin::default();
    let nested = MalformedInputBuilder::for_language("go").deeply_nested(depths::MEDIUM);

    let result = run_with_stack(move || plugin.parse_ast(&nested));

    match result {
        StackSafeResult::Ok(_) | StackSafeResult::Panicked(_) => {
            // Parse may succeed or fail; deep nesting can still panic.
        }
    }
}
...
fn test_deeply_nested_deep() {
    let plugin = GoPlugin::default();
    let nested = MalformedInputBuilder::for_language("go").deeply_nested(depths::DEEP);
...
fn test_oversized_1mb() {
    let plugin = GoPlugin::default();
    let large = MalformedInputBuilder::for_language("go").oversized_1mb();

    // This is valid Go (repeated), so parse should succeed
...
fn test_oversized_10mb() {
    let plugin = GoPlugin::default();
    let large = MalformedInputBuilder::for_language("go").oversized_10mb();
...
fn test_random_bytes() {
    ...
    // Random bytes very unlikely to be valid Go, but tree-sitter is error-tolerant
    let _ = result;
}
...
fn test_build_graph_on_malformed() {
    ...
    if let Ok(tree) = plugin.parse_ast(&malformed) {
        let builder = plugin
            .graph_builder()
            .expect("Go plugin should provide GraphBuilder");
