...
/// Helper: Check if a node with given qualified name exists
fn node_exists(staging: &StagingGraph, qualified_name: &str) -> bool {
    let node_names = build_node_name_lookup(staging);
    node_names.values().any(|name| name == qualified_name)
}
