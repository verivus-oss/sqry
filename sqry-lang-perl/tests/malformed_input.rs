//! Malformed input tests for Perl language plugin.
//!
...
fn test_deeply_nested_shallow() {
    let plugin = PerlPlugin::default();
    let nested = MalformedInputBuilder::for_language("perl").deeply_nested(depths::SHALLOW);
...
fn test_deeply_nested_medium() {
    let plugin = PerlPlugin::default();
    let nested = MalformedInputBuilder::for_language("perl").deeply_nested(depths::MEDIUM);
...
fn test_deeply_nested_deep() {
    let plugin = PerlPlugin::default();
    let nested = MalformedInputBuilder::for_language("perl").deeply_nested(depths::DEEP);
...
fn test_oversized_1mb() {
    let plugin = PerlPlugin::default();
    let large = MalformedInputBuilder::for_language("perl").oversized_1mb();

    // This is valid Perl (repeated), so parse should succeed
...
fn test_oversized_10mb() {
    let plugin = PerlPlugin::default();
    let large = MalformedInputBuilder::for_language("perl").oversized_10mb();
...
fn test_random_bytes() {
    ...
    // Random bytes very unlikely to be valid Perl, but tree-sitter is error-tolerant
    let _ = result;
}
