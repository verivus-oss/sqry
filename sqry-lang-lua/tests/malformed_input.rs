//! Malformed input tests for Lua language plugin.
//!
...
fn test_deeply_nested_shallow() {
    let plugin = LuaPlugin::default();
    let nested = MalformedInputBuilder::for_language("lua").deeply_nested(depths::SHALLOW);
...
fn test_deeply_nested_medium() {
    let plugin = LuaPlugin::default();
    let nested = MalformedInputBuilder::for_language("lua").deeply_nested(depths::MEDIUM);
...
fn test_deeply_nested_deep() {
    let plugin = LuaPlugin::default();
    let nested = MalformedInputBuilder::for_language("lua").deeply_nested(depths::DEEP);
...
fn test_oversized_1mb() {
    let plugin = LuaPlugin::default();
    let large = MalformedInputBuilder::for_language("lua").oversized_1mb();

    // This is valid Lua (repeated), so parse should succeed
...
fn test_oversized_10mb() {
    let plugin = LuaPlugin::default();
    let large = MalformedInputBuilder::for_language("lua").oversized_10mb();
...
fn test_random_bytes() {
    ...
    // Random bytes very unlikely to be valid Lua, but tree-sitter is error-tolerant
    let _ = result;
}
