//! Malformed input tests for Css language plugin.
//!
...
fn test_deeply_nested_shallow() {
    let plugin = CssPlugin::default();
    let nested = MalformedInputBuilder::for_language("css").deeply_nested(depths::SHALLOW);
...
fn test_deeply_nested_medium() {
    let plugin = CssPlugin::default();
    let nested = MalformedInputBuilder::for_language("css").deeply_nested(depths::MEDIUM);
...
fn test_deeply_nested_deep() {
    let plugin = CssPlugin::default();
    let nested = MalformedInputBuilder::for_language("css").deeply_nested(depths::DEEP);
...
fn test_oversized_1mb() {
    let plugin = CssPlugin::default();
    let large = MalformedInputBuilder::for_language("css").oversized_1mb();

    // This is valid Css (repeated), so parse should succeed
...
fn test_oversized_10mb() {
    let plugin = CssPlugin::default();
    let large = MalformedInputBuilder::for_language("css").oversized_10mb();
...
fn test_random_bytes() {
    ...
    // Random bytes very unlikely to be valid Css, but tree-sitter is error-tolerant
    let _ = result;
}
...
// Note: Css is a markup/template language without relations (imports/exports/calls)
