...
fn staged_nodes(source: &str) -> Vec<(String, u32, u32, u32)> {
    ...
    sqry_lang_groovy::relations::GroovyGraphBuilder
        .build_graph(
            &tree,
            source.as_bytes(),
            Path::new("P.groovy"),
...
fn no_declaration_collapses_onto_line_one_with_a_byte_offset_column() {
    ...
    // Field declaration, converted from `Span::from_bytes(field_node...)`.
    let field = find(&nodes, "Holder::answer");
    assert_eq!(
        field.1, 10,
        "`private int answer` is on line 10, got line {} col {}",
        field.1, field.2
    );
