...
fn process_variable_typeof_edges(
    ...
    if has_direct_type_child && owner_class.is_some() {
        ...
        let qualified_name = format!("{}.{}", owner_class.unwrap(), variable_name);
...
/// Detect FFI annotation from marker_annotation + function_signature pattern
/// (used when tree-sitter doesn't create external_declaration nodes)
...
/// Extract symbol from a marker_annotation node directly
...
/// Returns true only for exact matches, not substrings (e.g., @NativeCallable returns false)
...
/// Extract function name from an ERROR node containing "external int functionName();"
...
/// Extract symbol name from annotation arguments.
...
void main() {
  ...
  static void increment() {
    count++;
        ...
        let static_field = staging
            .nodes()
            .find(|n| staging.resolve_node_name(n.entry) == Some("Counter.count"));
        assert!(
        ...
        let instance_field = staging
            .nodes()
            .find(|n| staging.resolve_node_name(n.entry) == Some("Counter.instance"));
        assert!(
