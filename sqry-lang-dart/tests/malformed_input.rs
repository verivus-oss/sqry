//! Malformed input tests for Dart language plugin.
//!
...
fn test_deeply_nested_shallow() {
    let plugin = DartPlugin::default();
    let nested = MalformedInputBuilder::for_language("dart").deeply_nested(depths::SHALLOW);
...
fn test_deeply_nested_medium() {
    let plugin = DartPlugin::default();
    let nested = MalformedInputBuilder::for_language("dart").deeply_nested(depths::MEDIUM);
...
fn test_deeply_nested_deep() {
    let plugin = DartPlugin::default();
    let nested = MalformedInputBuilder::for_language("dart").deeply_nested(depths::DEEP);
...
fn test_oversized_1mb() {
    let plugin = DartPlugin::default();
    let large = MalformedInputBuilder::for_language("dart").oversized_1mb();

    // This is valid Dart (repeated), so parse should succeed
...
fn test_oversized_10mb() {
    let plugin = DartPlugin::default();
    let large = MalformedInputBuilder::for_language("dart").oversized_10mb();
...
fn test_random_bytes() {
    ...
    // Random bytes very unlikely to be valid Dart, but tree-sitter is error-tolerant
    let _ = result;
}
