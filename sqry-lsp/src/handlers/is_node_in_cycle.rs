...
pub fn execute(
    ...
    let node_id = match snapshot.resolve_symbol(&SymbolQuery {
        symbol,
...
mod tests {
    ...
    #[test]
    fn calls_edge_relevant_for_default_cycle_type() {
    ...
    #[test]
    fn calls_edge_not_relevant_for_imports_cycle_type() {
        let kind = EdgeKind::Calls {
            argument_count: 0,
            is_async: false,
            resolved_via: ResolvedVia::Direct,
    ...
    #[test]
    fn imports_edge_relevant_for_imports_cycle_type() {
    ...
    #[test]
    fn imports_edge_not_relevant_for_calls_cycle_type() {
        let kind = EdgeKind::Imports {
            alias: None,
            is_wildcard: false,
        };
    ...
    #[test]
    fn all_cycle_type_includes_calls_and_imports() {
        ...
        assert!(is_relevant_edge(&calls, "all"));
        assert!(is_relevant_edge(&imports, "all"));
    }

    #[test]
    fn all_cycle_type_excludes_defines() {
        assert!(!is_relevant_edge(&EdgeKind::Defines, "all"));
    }

    #[test]
    fn unknown_cycle_type_falls_through_to_calls_branch() {
        // "modules" is unrecognized → falls to _ => matches Calls
        let kind = EdgeKind::Calls {
            argument_count: 0,
            is_async: false,
            resolved_via: ResolvedVia::Direct,
        ...
        assert!(!is_relevant_edge(&EdgeKind::References, "modules"));
    }
}
