//! Malformed input tests for Sql language plugin.
//!
...
fn test_deeply_nested_shallow() {
    let plugin = SqlPlugin::default();
    let nested = MalformedInputBuilder::for_language("sql").deeply_nested(depths::SHALLOW);
...
fn test_deeply_nested_medium() {
    let plugin = SqlPlugin::default();
    let nested = MalformedInputBuilder::for_language("sql").deeply_nested(depths::MEDIUM);
...
fn test_deeply_nested_deep() {
    let plugin = SqlPlugin::default();
    let nested = MalformedInputBuilder::for_language("sql").deeply_nested(depths::DEEP);
...
fn test_oversized_1mb() {
    let plugin = SqlPlugin::default();
    let large = MalformedInputBuilder::for_language("sql").oversized_1mb();

    // This is valid Sql (repeated), so parse should succeed
...
fn test_oversized_10mb() {
    let plugin = SqlPlugin::default();
    let large = MalformedInputBuilder::for_language("sql").oversized_10mb();
...
fn test_random_bytes() {
    ...
    // Random bytes very unlikely to be valid Sql, but tree-sitter is error-tolerant
    let _ = result;
}
...
fn test_build_graph_on_malformed() {
    ...
    if let Ok(tree) = plugin.parse_ast(&malformed) {
        let builder = plugin
            .graph_builder()
            .expect("SQL plugin should provide GraphBuilder");
