...
impl SqlQueries {
    ...
    fn new(language: &tree_sitter::Language) -> GraphResult<Self> {
        ...
        // Includes function invocations like: function_name(args) or schema.function_name(args)
        // Note: All invocation nodes use (object_reference name: (identifier)) pattern
    ...
    while let Some(m) = matches.next() {
        ...
        for capture in m.captures {
            ...
            match name {
                "call.name" => {
                    if let Ok(text) = capture.node.utf8_text(content) {
                        call_name = Some(text.trim().to_string());
                    }
                }
