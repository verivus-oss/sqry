...
> **Note**: The examples below assume `sqry` and `sqry-mcp` are on your `PATH` via `cargo install`. If you skip the install step, prefix commands with `./target/debug/` (e.g., `./target/debug/sqry index`).
...
# Use with Claude Code (add to .claude/settings.json):
...
```bash
cargo test --workspace           # All tests pass (9,100+)
