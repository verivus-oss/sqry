...
use sqry_core::graph::unified::{EdgeKind, NodeEntry, NodeId, StagingGraph, StringId};
...
pub fn build_string_map(staging: &StagingGraph) -> HashMap<StringId, String> {
    staging
        .operations()
        .iter()
        .filter_map(|op| {
...
pub fn build_node_name_map(staging: &StagingGraph) -> HashMap<NodeId, String> {
    ...
    for op in staging.operations() {
        if let StagingOp::AddNode { entry, expected_id } = op
            ...
            && let Some(name) = node_name(entry, &string_map)
        {
...
fn node_name(entry: &NodeEntry, strings: &HashMap<StringId, String>) -> Option<String> {
    strings.get(&entry.name).cloned()
}
