//! Malformed input tests for Javascript language plugin.
//!
...
fn test_deeply_nested_shallow() {
    let plugin = JavaScriptPlugin::default();
    let nested = MalformedInputBuilder::for_language("javascript").deeply_nested(depths::SHALLOW);
...
fn test_deeply_nested_medium() {
    let plugin = JavaScriptPlugin::default();
    let nested = MalformedInputBuilder::for_language("javascript").deeply_nested(depths::MEDIUM);
...
fn test_deeply_nested_deep() {
    let plugin = JavaScriptPlugin::default();
    let nested = MalformedInputBuilder::for_language("javascript").deeply_nested(depths::DEEP);
...
fn test_oversized_1mb() {
    let plugin = JavaScriptPlugin::default();
    let large = MalformedInputBuilder::for_language("javascript").oversized_1mb();

    // This is valid Javascript (repeated), so parse should succeed
...
fn test_oversized_10mb() {
    let plugin = JavaScriptPlugin::default();
    let large = MalformedInputBuilder::for_language("javascript").oversized_10mb();
...
fn test_random_bytes() {
    ...
    // Random bytes very unlikely to be valid Javascript, but tree-sitter is error-tolerant
    let _ = result;
}
...
#[test]
fn test_build_graph_on_malformed_ffi_calls() {
    let plugin = JavaScriptPlugin::default();
    let source = br#"
function load() {
    WebAssembly.instantiate(42);
    const native = require(123);
    process.dlopen(module, 456);
}
"#;

    let tree = plugin.parse_ast(source).expect("parse JS source");
    let builder = plugin.graph_builder().expect("graph builder");
    let mut staging = StagingGraph::new();
    let result = builder.build_graph(&tree, source, Path::new("ffi_malformed.js"), &mut staging);
    let _ = result;
}
