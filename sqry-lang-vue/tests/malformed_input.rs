//! Malformed input tests for Vue language plugin.
//!
...
fn test_deeply_nested_shallow() {
    let plugin = VuePlugin::default();
    let nested = MalformedInputBuilder::for_language("vue").deeply_nested(depths::SHALLOW);
...
fn test_deeply_nested_medium() {
    let plugin = VuePlugin::default();
    let nested = MalformedInputBuilder::for_language("vue").deeply_nested(depths::MEDIUM);
...
fn test_deeply_nested_deep() {
    let plugin = VuePlugin::default();
    let nested = MalformedInputBuilder::for_language("vue").deeply_nested(depths::DEEP);
...
fn test_oversized_1mb() {
    let plugin = VuePlugin::default();
    let large = MalformedInputBuilder::for_language("vue").oversized_1mb();

    // This is valid Vue (repeated), so parse should succeed
...
fn test_oversized_10mb() {
    let plugin = VuePlugin::default();
    let large = MalformedInputBuilder::for_language("vue").oversized_10mb();
...
fn test_random_bytes() {
    ...
    // Random bytes very unlikely to be valid Vue, but tree-sitter is error-tolerant
    let _ = result;
}
...
fn test_build_graph_on_malformed() {
    ...
    if let Ok(tree) = plugin.parse_ast(&malformed) {
        let builder = plugin
            .graph_builder()
            .expect("Vue plugin should provide GraphBuilder");
...
// Note: Vue is a markup/template language without relations (imports/exports/calls)
