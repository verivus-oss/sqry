...
/// Fast byte-level pre-check that rejects non-JSON content in <1μs.
///
/// Scans the first bytes of content (skipping UTF-8 BOM and whitespace)
/// to verify the first significant byte is `{` or `[`, the only valid
/// JSON root tokens.  Rejects binary files and non-JSON content early
/// before the tree-sitter parse allocates memory.
fn json_fast_precheck(content: &[u8]) -> bool {
    // Skip UTF-8 BOM if present
    let start = if content.starts_with(&[0xEF, 0xBB, 0xBF]) {
        3
    } else {
        0
    };
...
impl GraphBuilder for JsonGraphBuilder {
    fn build_graph(
        ...
        // Fast pre-check: reject files that don't start with `{` or `[`
        // (after skipping UTF-8 BOM and whitespace).  This rejects binary
        // files or non-JSON content misnamed with a `.json` extension in <1μs.
        if !json_fast_precheck(content) {
            return Ok(());
        }
...
mod tests {
    ...
    #[test]
    fn test_json_precheck_object() {
        assert!(json_fast_precheck(b"{}"));
        assert!(json_fast_precheck(b" { \"key\": 1 }"));
        assert!(json_fast_precheck(b"\n\t{\"a\":1}"));
    }

    #[test]
    fn test_json_precheck_array() {
        assert!(json_fast_precheck(b"[1,2,3]"));
        assert!(json_fast_precheck(b"  [\"a\"]"));
    }

    #[test]
    fn test_json_precheck_with_bom() {
        assert!(json_fast_precheck(b"\xEF\xBB\xBF{\"key\":1}"));
        assert!(json_fast_precheck(b"\xEF\xBB\xBF [1]"));
    }

    #[test]
    fn test_json_precheck_rejects_binary() {
        assert!(!json_fast_precheck(b"\x00\x01\x02binary"));
        assert!(!json_fast_precheck(b"not json at all"));
        assert!(!json_fast_precheck(b""));
    }
