...
//! It uses a DistilBERT-based intent classifier combined with regex-based entity
...
//! 3. **Intent Classifier** - `DistilBERT` ONNX model for intent classification
...
//! ## Feature Flags
//!
//! - `classifier` (default) - Enable the `DistilBERT` classifier. Requires ONNX Runtime.
//!   Disable for minimal builds that only need preprocessing and validation.
