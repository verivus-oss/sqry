...
pub struct TranslatorConfig {
    ...
    /// Directory should contain: model.onnx, tokenizer.json, and optionally calibration.json
...
impl Translator {
    ...
    fn generate_disambiguation_options(entities: &ExtractedEntities) -> Vec<DisambiguationOption> {
        ...
        if let Some(symbol) = entities.primary_symbol() {
            ...
            options.push(DisambiguationOption {
                command: format!("sqry graph callers \"{symbol}\""),
        ...
        match error {
            ...
            AssemblerError::AmbiguousIntent => TranslationResponse::Disambiguate {
                options: vec![
                    ...
                    DisambiguationOption {
                        command: "sqry graph callers \"<symbol>\"".to_string(),
                        intent: Intent::FindCallers,
...
mod tests {
    ...
    fn test_translate_callers() {
        ...
        match response {
            ...
            TranslationResponse::Confirm { command, .. } => {
                // Confirm doesn't carry intent, but should have graph callers command
                assert!(command.contains("graph callers") || command.contains("authenticate"));
            }
            _ => {}
        }
    }
