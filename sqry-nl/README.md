...
|---------|---------|-------------|
| `classifier` | Yes | Enable DistilBERT ONNX classifier (requires ONNX Runtime) |
...
| `FindCallers` | "who calls authenticate" | `sqry graph callers "authenticate"` |
| `FindCallees` | "what does main call" | `sqry graph callees "main"` |
...
# 2. Train model
python train_classifier.py train --train data/train.json --output models/classifier

# 3. Export to ONNX with quantization
python export_onnx.py export --model models/classifier/final --output models/onnx --quantize

# 4. Calibrate confidence
python calibrate.py calibrate --model models/onnx/model.onnx --data data/eval.json

# 5. Copy to sqry-nl/models/
cp models/onnx/quantized/model.onnx ../models/
```
