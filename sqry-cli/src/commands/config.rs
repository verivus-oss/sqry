...
fn set_config_value(config: &mut GraphConfigFile, key: &str, value: &str) -> Result<()> {
    ...
    match parts[0] {
        "limits" => match parts.get(1) {
            ...
            Some(&"analysis_label_budget_per_kind") => {
                config.config.limits.analysis_label_budget_per_kind = value
                    .parse()
                    .context("Invalid value for analysis_label_budget_per_kind (expected u64)")?;
            }
            Some(&"analysis_density_gate_threshold") => {
                config.config.limits.analysis_density_gate_threshold = value
                    .parse()
                    .context("Invalid value for analysis_density_gate_threshold (expected u64)")?;
            }
            Some(&"analysis_budget_exceeded_policy") => {
                if !["degrade", "fail"].contains(&value) {
                    anyhow::bail!(
                        "Invalid analysis_budget_exceeded_policy (expected: degrade or fail)"
                    );
                }
