...
fn format_csv_output_shared(
    ...
    // Write header
    if cli.headers {
        let headers: Vec<&str> = columns.iter().map(column_header).collect();
...
/// Convert CsvColumn to DiffColumn (for --columns flag support)
...
///
/// Uses same logic as shared CsvFormatter
...
mod tests {
    ...
    fn test_diff_column_headers() {
        ...
        assert_eq!(
            column_header(&DiffColumn::SignatureAfter),
            "signature_after"
        );
    }
