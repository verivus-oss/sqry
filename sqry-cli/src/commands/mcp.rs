    ...
    match scope {
        SetupScope::Project | SetupScope::Auto => {
            ...
            if dry_run {
                ...
                println!("  Path: projects[\"{}\"].mcpServers.sqry", root_str);
...
fn write_claude_project_entry(
    ...
    if servers.contains_key("sqry") && !force {
        bail!(
            "Claude Code project entry for sqry already exists at projects[\"{}\"].\n\
...
fn print_status_human(binary: &str) -> Result<()> {
...
fn print_gemini_status_human() -> Result<()> {
    let config_path = match gemini_config_path() {
        Some(p) => p,
        None => {
...
fn claude_status_json() -> Result<Value> {
    let config_path = match claude_config_path() {
        Some(p) => p,
        None => return Ok(serde_json::json!({"configured": false})),
    };
    ...
    let global = config
        ...
        .unwrap_or_else(|| serde_json::json!({"configured": false}));
