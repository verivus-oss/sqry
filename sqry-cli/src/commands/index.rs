...
pub fn run_index(
    ...
    let build_config =
        create_build_config_with_label_budget(cli, threads, label_budget, label_budget_fail);
    let _build_result = step_runner.step("Build unified graph", || -> Result<_> {
    ...
    if !cli.json {
        ...
        emit_graph_summary(&status, elapsed);
    }

    Ok(())
}

fn emit_graph_summary(status: &IndexStatus, elapsed: std::time::Duration) {
...
pub(crate) fn create_build_config(
    ...
    create_build_config_with_label_budget(cli, threads, None, false)
}

/// Create a `BuildConfig` from CLI flags with optional label budget overrides.
pub(crate) fn create_build_config_with_label_budget(
    cli: &Cli,
    threads: Option<usize>,
    label_budget: Option<usize>,
    label_budget_fail: bool,
) -> sqry_core::graph::unified::build::BuildConfig {
    let mut config = sqry_core::graph::unified::build::BuildConfig {
    ...
    // CLI flags override env vars (which are already read in BuildConfig::default())
    if let Some(budget) = label_budget {
        config.label_budget.budget_per_kind = budget;
    }
    if label_budget_fail {
        config.label_budget.on_exceeded =
            sqry_core::graph::unified::analysis::BudgetExceededPolicy::Fail;
    }
...
pub fn run_update(
    ...
    let _build_result = step_runner.step("Update unified graph", || -> Result<_> {
        let (_graph, build_result) =
            sqry_core::graph::unified::build::build_and_persist_graph_with_progress(
                root_path,
                &sqry_plugin_registry::create_plugin_manager(),
    ...
    if !cli.json {
        ...
        println!("  Time taken: {:.2}s", elapsed.as_secs_f64());
        if let Some(ref idx_path) = status.path {
            println!("  Index saved to: {idx_path}");
        }
    }
