...
pub fn run_index(
    ...
    let build_config = create_build_config(cli);
...
pub(crate) fn create_build_config(cli: &Cli) -> sqry_core::graph::unified::build::BuildConfig {
...
pub fn run_update(
    ...
    if !cli.json {
        ...
        // For now, always report "updated" since we do a full rebuild
        // In a future optimization, we could use git diff to determine actual changes
        // and only report "unchanged" when there are truly no changes
        if using_git_mode {
    ...
    if show_stats {
        println!("(Detailed stats not yet implemented for unified graph update)");
    }
...
#[allow(deprecated)]
/// Run index status command for LLM agents.
///
/// # Arguments
///
