...
fn handle_cli_error(cli_error: &error::CliError, json_output: bool) -> i32 {
    ...
    if json_output {
        let code = match cli_error {
            ...
            error::CliError::PagerExit(_) => unreachable!(), // handled above
        };
...
fn run() -> Result<()> {
    ...
    match &cli.command {
        ...
        Some(Command::Analyze { path, force }) => {
            commands::run_analyze(&cli, path.as_deref(), *force)
        ...
        Some(Command::Watch {
            ...
            commands::run_watch(&cli, path.clone(), *debounce, *stats, *build)
    ...
    match validate {
        ValidationMode::Fail if is_stale => {
            if auto_rebuild {
                ...
                if let Err(err) =
                    commands::run_index(cli, search_path, true, None, false, false, None, false)
                {
...
fn handle_index_command(
    ...
    if status {
        ...
        // Always build analysis (CSR, SCC, condensation DAG) after indexing.
        // This ensures find_unused, find_cycles, and other analysis tools work
        // without requiring a separate `sqry analyze` step.
        commands::run_analyze(cli, Some(index_path), true)
            .context("Analysis after indexing failed")?;
    }
    Ok(())
}
