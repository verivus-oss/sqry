...
pub enum Command {
    ...
    Index {
        ...
        /// Maximum 2-hop label budget per edge kind (default: 50,000,000).
        ///
        /// Controls the maximum number of reachability intervals computed
        /// during analysis. Larger budgets allow faster reachability queries
        /// but use more memory. Override via SQRY_LABEL_BUDGET env var.
        #[arg(long = "label-budget", help_heading = headings::ADVANCED_CONFIGURATION, display_order = 30)]
        label_budget: Option<usize>,

        /// Fail the build if the label budget is exceeded (default: degrade to BFS).
        ///
        /// By default, when the label budget is exceeded for an edge kind,
        /// reachability degrades to BFS fallback. Set this flag to fail instead.
        /// Override via SQRY_LABEL_BUDGET_FAIL=1 env var.
        #[arg(long = "label-budget-fail", help_heading = headings::ADVANCED_CONFIGURATION, display_order = 31)]
