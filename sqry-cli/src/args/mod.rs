...
pub enum Command {
    ...
    Index {
        ...
        ///
        /// Reads the per-crate JSON cache produced by `sqry cache expand` and
        /// materialises macro-generated Rust symbols (derive impls, generated
        /// methods, etc.) as searchable graph nodes flagged `macro_generated`.
    ...
    /// Generate a high-signal orientation report for an unfamiliar repository
    ///
    /// Composes shipped analyses into one report: a summary (health + stats),
    /// load-bearing hubs, path/package subsystems and their couplings, complexity
    /// hotspots, potential issues (cycles, unused public APIs, duplicates, high
    /// fan-out), and a set of ready-to-run sqry queries. Every finding ends by
    /// pointing at a concrete query to run next. The report path is integer and
    /// deterministic, so `--format json` on a fixed snapshot is byte-stable.
    ///
    /// Examples:
    ///   sqry overview                          # Markdown report to stdout
    ///   sqry overview --format json            # Stable JSON serialization
    ///   sqry overview --sections hubs,issues   # Only the named sections
    ///   sqry overview --top 5 --output MAP.md  # Bound rows, write to a file
    #[command(alias = "report", display_order = 24, verbatim_doc_comment)]
    Overview {
        /// Search path (defaults to current directory).
        #[arg(help_heading = headings::SEARCH_INPUT, display_order = 10)]
        path: Option<String>,
        ...
        /// - json: stable-field-order JSON serialization
        /// - text: terse console digest
        #[arg(long, short = 'f', default_value = "md", value_parser = ["md", "json", "text"], help_heading = headings::OUTPUT_CONTROL, display_order = 10)]
        format: String,

        /// Maximum rows per ranked section (default: 10).
        #[arg(long, default_value = "10", help_heading = headings::OUTPUT_CONTROL, display_order = 20)]
        top: usize,

        /// Comma-separated subset of sections to include.
        ///
        /// Section names: summary, hubs, subsystems, hotspots, issues,
        /// questions. Defaults to all sections in report order.
        #[arg(long, help_heading = headings::OUTPUT_CONTROL, display_order = 30)]
        sections: Option<String>,

        /// Number of leading directory components that form a subsystem bucket
        /// key (default: 2).
        #[arg(long, default_value = "2", help_heading = headings::GRAPH_ANALYSIS_OPTIONS, display_order = 10)]
        group_depth: usize,

        /// Write the report to FILE instead of stdout (stdout stays silent).
        #[arg(long, short = 'o', value_name = "FILE", help_heading = headings::OUTPUT_CONTROL, display_order = 40)]
        output: Option<PathBuf>,

        /// Fail if the graph is missing or stale rather than building it.
        #[arg(long, help_heading = headings::SEARCH_INPUT, display_order = 20)]
        no_index: bool,

        /// Path/name redaction preset applied to the report.
        ///
        /// - minimal: workspace-relative paths, code/names preserved (default)
        /// - none: no redaction (raw absolute paths; trusted local use only)
        /// - relative: clean workspace-relative layout
        #[arg(long, default_value = "minimal", value_parser = ["minimal", "none", "relative"], help_heading = headings::OUTPUT_CONTROL, display_order = 50)]
        redaction: String,
    },
...
pub enum GraphOperation {
    ...
    /// Rank the most load-bearing symbols (degree-centrality hubs)
    ///
    /// Ranks symbols by degree centrality over Calls + References edges.
    /// `fan_in` is the count of incoming edges (most depended-upon core APIs),
    /// `fan_out` the count of outgoing edges (broad orchestrators). The whole
    /// path is integer, so the ranking is byte-stable across runs.
    ///
    /// Example: sqry graph hubs --top 20 --by combined
    Hubs {
        /// Maximum number of hubs to return (default: 10; 0 = unbounded).
        #[arg(long, default_value = "10", help_heading = headings::GRAPH_OUTPUT_OPTIONS, display_order = 10)]
        top: usize,

        /// Which degree metric ranks the hubs.
        ///
        /// - fan-in: most depended-upon symbols (default)
        /// - fan-out: broadest orchestrators
        /// - combined: true hubs (fan_in * fan_out)
        #[arg(long, default_value = "fan-in", value_parser = ["fan-in", "fan-out", "combined"], help_heading = headings::GRAPH_ANALYSIS_OPTIONS, display_order = 10)]
        by: String,
    },

    /// Aggregate the codebase into path/package subsystems and their couplings
    ///
    /// Groups symbols into directory-prefix subsystems (at `--group-depth`) and
    /// reports the directed couplings between them, surfacing where the
    /// dependency structure disagrees with the directory layout. Integer counts
    /// over sorted keys, so the output is byte-stable across runs.
    ///
    /// Example: sqry graph subsystems --top 15 --group-depth 3
    Subsystems {
        /// Maximum subsystems and couplings to return (default: 10; 0 = unbounded).
        #[arg(long, default_value = "10", help_heading = headings::GRAPH_OUTPUT_OPTIONS, display_order = 10)]
        top: usize,

        /// Number of leading directory components that form a bucket key (default: 2).
        #[arg(long, default_value = "2", help_heading = headings::GRAPH_ANALYSIS_OPTIONS, display_order = 10)]
        group_depth: usize,
    },

    /// Detect dependency-density communities (coarsened in-house Louvain)
    ///
    /// An optional power-user primitive: finds file-level dependency clusters
    /// that can cut across the directory layout, using exact-integer Louvain on
    /// the file-coarsened Calls + References graph. Deterministic; over a fixed
    /// structural gate it returns a "graph too large" verdict rather than
    /// depending on wall-clock. Not on the `sqry overview` report path.
    ///
    /// Example: sqry graph communities --top 10 --resolution 1.5
    Communities {
        /// Maximum communities to return (default: 10; 0 = unbounded).
        #[arg(long, default_value = "10", help_heading = headings::GRAPH_OUTPUT_OPTIONS, display_order = 10)]
        top: usize,

        /// Reichardt-Bornholdt resolution gamma (default: 1.0). Higher values
        /// yield more, smaller communities.
        #[arg(long, default_value = "1.0", help_heading = headings::GRAPH_ANALYSIS_OPTIONS, display_order = 10)]
        resolution: f64,
    },
}
...
mod tests {
    ...
    large_stack_test! {
    #[test]
    fn test_index_still_rejects_unwired_macro_flags() {
        // `--cfg` (Phase 1a) and `--expand-cache` (Phase 1b) are restored and
        // wired; only `--enable-macro-expansion` (live `cargo expand`, Phase 2)
        // stays removed, so clap must keep rejecting it as an unknown argument
        // rather than silently accept a no-op.
        assert!(
            Cli::try_parse_from(["sqry", "index", "--enable-macro-expansion"]).is_err(),
            "unwired --enable-macro-expansion must be rejected"
        );
    }
    }
    ...
    large_stack_test! {
    #[test]
    fn test_index_expand_cache_flag_parses() {
        // Phase 1b: `--expand-cache <DIR>` is restored on `index` and parses
        // into the `expand_cache` option, feeding the F4 materialisation
        // consumer.
        let cli = Cli::try_parse_from(["sqry", "index", "--expand-cache", "/tmp/expand"])
            .expect("--expand-cache must parse on index");
        match cli.command.as_deref() {
            Some(Command::Index { expand_cache, .. }) => {
                assert_eq!(
                    expand_cache.as_deref(),
                    Some(std::path::Path::new("/tmp/expand"))
                );
            }
    ...
    large_stack_test! {
    #[test]
    fn test_index_cfg_flag_parses_into_cfg_flags() {
        // Phase 1a: `--cfg` is restored on `index` and parses (repeatably) into
        // the `cfg_flags` vec, feeding the Rust macro-boundary cfg analysis.
        let cli = Cli::try_parse_from([
            "sqry", "index", "--cfg", "test", "--cfg", "feature=serde",
        ])
        .expect("--cfg must parse on index");
        match cli.command.as_deref() {
            Some(Command::Index { cfg_flags, .. }) => {
                assert_eq!(
                    *cfg_flags,
