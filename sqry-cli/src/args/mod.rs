...
pub enum Command {
    ...
    /// The index is saved to .sqry/ and speeds up subsequent queries.
    /// Uses parallel processing by default for faster indexing.
    ///
    /// Run `sqry analyze` to precompute analysis artifacts (SCC, condensation
    /// DAGs, 2-hop labels) for faster cycle detection and reachability queries.
    ...
    ///
    /// Note: `sqry index` builds the graph and CSR adjacency. Run `sqry analyze`
    /// separately to precompute SCC, condensation DAGs, and 2-hop labels for
    /// faster cycle detection and reachability queries.
    ///
    /// Examples:
