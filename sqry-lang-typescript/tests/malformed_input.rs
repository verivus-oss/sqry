//! Malformed input tests for Typescript language plugin.
//!
...
fn test_deeply_nested_shallow() {
    let plugin = TypeScriptPlugin::default();
    let nested = MalformedInputBuilder::for_language("typescript").deeply_nested(depths::SHALLOW);
...
fn test_deeply_nested_medium() {
    let plugin = TypeScriptPlugin::default();
    let nested = MalformedInputBuilder::for_language("typescript").deeply_nested(depths::MEDIUM);
...
fn test_deeply_nested_deep() {
    let plugin = TypeScriptPlugin::default();
    let nested = MalformedInputBuilder::for_language("typescript").deeply_nested(depths::DEEP);
...
fn test_oversized_1mb() {
    let plugin = TypeScriptPlugin::default();
    let large = MalformedInputBuilder::for_language("typescript").oversized_1mb();

    // This is valid Typescript (repeated), so parse should succeed
...
fn test_oversized_10mb() {
    let plugin = TypeScriptPlugin::default();
    let large = MalformedInputBuilder::for_language("typescript").oversized_10mb();
...
fn test_random_bytes() {
    ...
    // Random bytes very unlikely to be valid Typescript, but tree-sitter is error-tolerant
    let _ = result;
}
