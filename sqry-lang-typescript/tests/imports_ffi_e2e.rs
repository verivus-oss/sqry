...
fn build_node_lookup(staging: &StagingGraph) -> HashMap<NodeId, (String, NodeKind)> {
    ...
    for op in staging.operations() {
        ...
        {
            let name = names.get(node_id).cloned().unwrap_or_default();
    ...
    staging.operations().iter().any(|op| {
        if let StagingOp::AddNode { entry, .. } = op {
            entry.kind == expected_kind
                ...
                && staging
                    .resolve_node_display_name(Language::TypeScript, entry)
                    .as_deref()
...
"#;
    ...
    assert!(
        ...
        "synthetic wasm targets should remain canonical at display boundaries"
    );
    assert!(
        ffi_targets
            .iter()
    ...
    assert!(
        ...
        "synthetic native addon targets should remain canonical at display boundaries"
    );
    assert!(
    ...
    assert!(
        ...
        "synthetic dlopen targets should remain canonical at display boundaries"
    );
}
