//! Malformed input tests for Rust language plugin.
//!
...
fn test_deeply_nested_shallow() {
    let plugin = RustPlugin::default();
    let nested = MalformedInputBuilder::for_language("rust").deeply_nested(depths::SHALLOW);
...
fn test_deeply_nested_medium() {
    let plugin = RustPlugin::default();
    let nested = MalformedInputBuilder::for_language("rust").deeply_nested(depths::MEDIUM);
...
fn test_deeply_nested_deep() {
    let plugin = RustPlugin::default();
    let nested = MalformedInputBuilder::for_language("rust").deeply_nested(depths::DEEP);
...
fn test_oversized_1mb() {
    let plugin = RustPlugin::default();
    let large = MalformedInputBuilder::for_language("rust").oversized_1mb();

    // This is valid Rust (repeated), so parse should succeed
...
fn test_oversized_10mb() {
    let plugin = RustPlugin::default();
    let large = MalformedInputBuilder::for_language("rust").oversized_10mb();
...
fn test_random_bytes() {
    ...
    // Random bytes very unlikely to be valid Rust, but tree-sitter is error-tolerant
    let _ = result;
}
