...
/// Handle `secondary_constructor` with triple fallback: statements → function_body → block body loop.
...
/// Handle `try_expression`: `statements` child gets a TryBlock scope, others recurse
...
/// Handle `class_declaration` | `object_declaration`: class_stack push/pop with
