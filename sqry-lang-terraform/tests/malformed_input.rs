//! Malformed input tests for Terraform language plugin.
//!
...
fn test_deeply_nested_shallow() {
    let plugin = TerraformPlugin::default();
    let nested = MalformedInputBuilder::for_language("terraform").deeply_nested(depths::SHALLOW);
...
fn test_deeply_nested_medium() {
    let plugin = TerraformPlugin::default();
    let nested = MalformedInputBuilder::for_language("terraform").deeply_nested(depths::MEDIUM);
...
fn test_deeply_nested_deep() {
    let plugin = TerraformPlugin::default();
    let nested = MalformedInputBuilder::for_language("terraform").deeply_nested(depths::DEEP);
...
fn test_oversized_1mb() {
    let plugin = TerraformPlugin::default();
    let large = MalformedInputBuilder::for_language("terraform").oversized_1mb();

    // This is valid Terraform (repeated), so parse should succeed
...
fn test_oversized_10mb() {
    let plugin = TerraformPlugin::default();
    let large = MalformedInputBuilder::for_language("terraform").oversized_10mb();
...
fn test_random_bytes() {
    ...
    // Random bytes very unlikely to be valid Terraform, but tree-sitter is error-tolerant
    let _ = result;
}
