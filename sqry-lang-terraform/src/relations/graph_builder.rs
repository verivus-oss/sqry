...
fn collect_exportable_block(
    ...
    match bt.as_str() {
        ...
        "resource" => {
            ...
            // Canonical naming: <type>.<name> (e.g., "aws_instance.web") prevents
        ...
        "data" => {
            ...
            // Use canonical Terraform address: data.<type>.<name> for the variable node
            // This prevents collisions when different data types share the same local name
            if block_labels.len() >= 2 {
...
"#;
        ...
        assert_eq!(
            ...
            "Source should use canonical <type>.<name> format"
        );
        assert_eq!(
...
"#;
        ...
        assert_eq!(
            ...
            "Source should use canonical data.<type>.<name> format"
        );
        assert_eq!(
            details[0].1, "aws_ami",
...
"#;
        ...
        assert!(
            ...
            "Should have data.aws_ami.latest"
        );
        assert!(
            source_names.contains(&"data.aws_subnet.latest"),
            "Should have data.aws_subnet.latest"
        );
    }
