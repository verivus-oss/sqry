//! Malformed input tests for Zig language plugin.
//!
...
fn test_deeply_nested_shallow() {
    let plugin = ZigPlugin::default();
    let nested = MalformedInputBuilder::for_language("zig").deeply_nested(depths::SHALLOW);
...
fn test_deeply_nested_medium() {
    let plugin = ZigPlugin::default();
    let nested = MalformedInputBuilder::for_language("zig").deeply_nested(depths::MEDIUM);
...
fn test_deeply_nested_deep() {
    let plugin = ZigPlugin::default();
    let nested = MalformedInputBuilder::for_language("zig").deeply_nested(depths::DEEP);
...
fn test_oversized_1mb() {
    let plugin = ZigPlugin::default();
    let large = MalformedInputBuilder::for_language("zig").oversized_1mb();

    // This is valid Zig (repeated), so parse should succeed
...
fn test_oversized_10mb() {
    let plugin = ZigPlugin::default();
    let large = MalformedInputBuilder::for_language("zig").oversized_10mb();
...
fn test_random_bytes() {
    ...
    // Random bytes very unlikely to be valid Zig, but tree-sitter is error-tolerant
    let _ = result;
}
