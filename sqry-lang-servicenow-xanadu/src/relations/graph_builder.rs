...
//! - `gs.*` API calls such as logging, info, and error reporting
...
//! - Raw module source text as target, with no `path` resolution; `ServiceNow` scripts
...
struct GlideRecordContext {
    ...
    /// for example `gr -> incident` and `taskGR -> sc_task`.
...
///
/// Uses raw module source text without `resolve_import_path` because `ServiceNow` scripts
...
/// Handle `require()` calls, creating an `Import` edge without a `Call` edge.
///
