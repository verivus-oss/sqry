...
fn has_call_edge_display(staging: &StagingGraph, from: &str, to: &str) -> bool {
    let nodes = build_node_display_lookup(staging);
    for op in staging.operations() {
        if let StagingOp::AddEdge {
            source,
            target,
...
";
    ...
    assert!(
        has_call_edge_display(&staging, "MyClass.caller", "MyClass.callee"),
        "Expected native display call edge from caller method to callee method"
    );
}
