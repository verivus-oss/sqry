//! Malformed input tests for Xanadu language plugin.
//!
...
fn test_deeply_nested_shallow() {
    let plugin = ServiceNowXanaduPlugin::default();
    let nested = MalformedInputBuilder::for_language("xanadu").deeply_nested(depths::SHALLOW);
...
fn test_deeply_nested_medium() {
    let plugin = ServiceNowXanaduPlugin::default();
    let nested = MalformedInputBuilder::for_language("xanadu").deeply_nested(depths::MEDIUM);
...
fn test_deeply_nested_deep() {
    let plugin = ServiceNowXanaduPlugin::default();
    let nested = MalformedInputBuilder::for_language("xanadu").deeply_nested(depths::DEEP);
...
fn test_oversized_1mb() {
    let plugin = ServiceNowXanaduPlugin::default();
    let large = MalformedInputBuilder::for_language("xanadu").oversized_1mb();

    // This is valid Xanadu (repeated), so parse should succeed
...
fn test_oversized_10mb() {
    let plugin = ServiceNowXanaduPlugin::default();
    let large = MalformedInputBuilder::for_language("xanadu").oversized_10mb();
...
fn test_random_bytes() {
    ...
    // Random bytes very unlikely to be valid Xanadu, but tree-sitter is error-tolerant
    let _ = result;
}
