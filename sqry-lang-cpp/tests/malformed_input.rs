//! Malformed input tests for Cpp language plugin.
//!
...
fn test_deeply_nested_shallow() {
    let plugin = CppPlugin::default();
    let nested = MalformedInputBuilder::for_language("cpp").deeply_nested(depths::SHALLOW);
...
fn test_deeply_nested_medium() {
    let plugin = CppPlugin::default();
    let nested = MalformedInputBuilder::for_language("cpp").deeply_nested(depths::MEDIUM);
...
fn test_deeply_nested_deep() {
    let plugin = CppPlugin::default();
    let nested = MalformedInputBuilder::for_language("cpp").deeply_nested(depths::DEEP);
...
fn test_oversized_1mb() {
    let plugin = CppPlugin::default();
    let large = MalformedInputBuilder::for_language("cpp").oversized_1mb();

    // This is valid Cpp (repeated), so parse should succeed
...
fn test_oversized_10mb() {
    let plugin = CppPlugin::default();
    let large = MalformedInputBuilder::for_language("cpp").oversized_10mb();
...
fn test_random_bytes() {
    ...
    // Random bytes very unlikely to be valid Cpp, but tree-sitter is error-tolerant
    let _ = result;
}
