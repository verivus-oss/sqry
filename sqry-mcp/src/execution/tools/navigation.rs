...
pub fn execute_get_definition(
    ...
    for (_node_id, entry) in graph.nodes().iter() {
        ...
        if name == args.symbol
            || qualified_name == args.symbol
            || qualified_name.ends_with(&format!("::{}", args.symbol))
            || qualified_name.ends_with(&format!(".{}", args.symbol))
        {
            let file_path = files
                .resolve(entry.file)
...
fn find_target_nodes(
    ...
    for (node_id, entry) in graph.nodes().iter() {
        ...
        if name == symbol
            ...
            || qualified_name.ends_with(&suffix_colon)
            || qualified_name.ends_with(&suffix_dot)
        {
...
pub fn execute_get_references(
    ...
    let target_node_ids = find_target_nodes(&graph, &args.symbol);
...
pub fn execute_get_hover_info(args: &GetHoverInfoArgs) -> Result<ToolExecution<HoverInfoData>> {
    ...
    // Find the symbol (exact match or suffix match for unqualified names)
    let suffix_colon = format!("::{}", args.symbol);
    let suffix_dot = format!(".{}", args.symbol);
    for (_node_id, entry) in graph.nodes().iter() {
        let name = strings
            .resolve(entry.name)
            .map(|s| s.to_string())
    ...
    anyhow::bail!("Symbol '{}' not found", args.symbol)
}
