...
pub fn execute_semantic_search(
    ...
    filtered.sort_by(|&a, &b| {
        let name_a = snapshot.get_node(a).and_then(|e| strings.resolve(e.name));
        let name_b = snapshot.get_node(b).and_then(|e| strings.resolve(e.name));
        name_a.cmp(&name_b)
    });
...
fn find_reference_node(
    ...
    let reference_node_id = snapshot
        .nodes_by_symbol(symbol_name)
        .into_iter()
        .find(|&node_id| {
            if let Some(entry) = snapshot.get_node(node_id) {
                let node_file = snapshot.files().resolve(entry.file);
                node_file.is_some_and(|f| f.as_ref() == relative_file)
            } else {
                false
            }
        })
        .or_else(|| snapshot.nodes_by_symbol(symbol_name).first().copied())
        .ok_or_else(|| {
            anyhow!(
                "Symbol '{}' not found in {}",
                symbol_name,
...
pub fn execute_find_similar(args: &SearchSimilarArgs) -> Result<ToolExecution<FindSimilarData>> {
    ...
    for (node_id, entry) in snapshot.iter_nodes() {
        ...
        let candidate_name = strings
            ...
            .map(|s| s.to_string())
            .unwrap_or_default();
        let candidate_qualified_name = entry
            .qualified_name
            .and_then(|sid| strings.resolve(sid))
            .map_or_else(|| candidate_name.clone(), |s| s.to_string());

        // Skip already seen
        if seen_names.contains(&candidate_qualified_name) {
            continue;
        }
        seen_names.insert(candidate_qualified_name.clone());
        ...
        let candidate_ref = crate::execution::types::NodeRefData {
            name: candidate_name,
            qualified_name: candidate_qualified_name,
