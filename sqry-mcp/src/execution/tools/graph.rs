...
/// Compute dependencies using the unified graph (FR-2025-007).
...
pub fn execute_export_graph(args: &ExportGraphArgs) -> Result<ToolExecution<DependencyGraphData>> {
    ...
    let rendered = render_export_graph(args, Some(graph.clone()));
...
// ============================================================================
// Unified Graph Subgraph Computation (FR-2025-007)
// ============================================================================
...
mod tests {
    ...
    fn create_test_graph() -> CodeGraph {
        ...
        let _main_id = nodes.alloc(main_entry).unwrap();
        ...
        let _helper_id = nodes.alloc(helper_entry).unwrap();

        CodeGraph::from_components(nodes, edges, strings, files, indices)
    }
