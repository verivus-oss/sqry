...
pub fn execute_find_cycles(args: &FindCyclesArgs) -> Result<ToolExecution<FindCyclesData>> {
    ...
    let edge_kind_name = match args.cycle_type {
        CycleType::Calls | CycleType::Modules => "calls", // Modules use call edges
    ...
    {
        // Fast path: extract cycles from SCC data
        extract_cycles_from_scc(&graph, &scc_data, &config)
    } else {
        // Slow path: query-time traversal (O(V+E))
        let node_count = graph.snapshot().nodes().len();
        if node_count > 100_000 {
            return Err(anyhow!(
                "Graph has {node_count} nodes — cycle detection without precomputed analyses would exceed timeout. \
                 Run `sqry index` to rebuild the graph with analysis files.",
            ));
        }
        tracing::warn!(
            "Precomputed SCC data not found. Using slow O(V+E) cycle detection. \
             Run `sqry index` to rebuild the graph with analysis files for fast cycle detection."
        );
...
fn find_similar_symbols(
    ...
    for (_node_id, entry) in snapshot.nodes().iter() {
        ...
        // Check for substring match or prefix match, excluding exact query match
        if (name_lower.contains(simple_query) || simple_query.starts_with(&name_lower))
            && name != query
        {
...
pub fn execute_direct_callees(
    ...
    let source_node_id = snapshot.find_by_name(&args.symbol).ok_or_else(|| {
        // Try to find similar symbols for a helpful error message
        let suggestions = find_similar_symbols(&snapshot, &args.symbol, 3);
        let suggestions_str = if suggestions.is_empty() {
            String::new()
