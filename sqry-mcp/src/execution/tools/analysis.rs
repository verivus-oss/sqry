...
pub fn execute_dependency_impact(
    ...
    let target_node_id = snapshot.find_by_name(&args.symbol).ok_or_else(|| {
        anyhow!(
...
fn resolve_cycle_node(
    ...
    let node_ids = snapshot.nodes_by_symbol(name);
...
pub fn execute_find_cycles(args: &FindCyclesArgs) -> Result<ToolExecution<FindCyclesData>> {
    ...
    let edge_kind_name = match args.cycle_type {
        ...
        CycleType::Imports | CycleType::Modules => "imports", // Modules use import adjacency
    };

    let storage = sqry_core::graph::unified::persistence::GraphStorage::new(&workspace_root);
    let snapshot = graph.snapshot();
    let cycles = if let Some(scc_data) =
        sqry_core::graph::unified::analysis::try_load_scc(&storage, &snapshot, edge_kind_name)
    {
        // Fast path: extract cycles from precomputed SCC data (requires `sqry analyze`)
        extract_cycles_from_scc(&graph, &scc_data, &config)
    } else {
        // Medium path: build CSR on-the-fly from loaded graph, then compute SCC via Tarjan
        // This works without `sqry analyze` — the graph is already in memory from `sqry index`.
        use sqry_core::graph::unified::analysis::csr::CsrAdjacency;
        use sqry_core::graph::unified::compaction::build::snapshot_edges;
        use sqry_core::graph::unified::edge::EdgeKind;

        let edge_kind = match args.cycle_type {
            CycleType::Calls => EdgeKind::Calls {
                argument_count: 0,
                is_async: false,
            },
            CycleType::Imports | CycleType::Modules => EdgeKind::Imports {
                alias: None,
                is_wildcard: false,
            },
        };
        ...
        // Build CSR from graph's edge store (O(E) to snapshot + O(E log E) to sort + build)
        let csr_result = {
            let forward_store = graph.edges().forward();
            let compaction_snapshot = snapshot_edges(&forward_store, node_count);
            drop(forward_store); // Release read lock before heavy computation
            CsrAdjacency::build_from_snapshot(&compaction_snapshot)
        };

        match csr_result {
            Ok(csr) => {
                match sqry_core::graph::unified::analysis::scc::SccData::compute_tarjan(
                    &csr, &edge_kind,
                ) {
                    Ok(scc_data) => extract_cycles_from_scc(&graph, &scc_data, &config),
                    Err(e) => {
                        tracing::warn!(
                            "SCC computation failed: {e}. Falling back to graph traversal."
                        );
                        if node_count > 100_000 {
                            return Err(anyhow!(
                                "Graph has {node_count} nodes and SCC computation failed: {e}. \
                                 Run `sqry analyze` to precompute cycle data.",
                            ));
                        }
            ...
            Err(e) => {
                // CSR build failed — fall back to direct traversal for small graphs
                tracing::warn!("CSR build failed: {e}. Falling back to graph traversal.");
                if node_count > 100_000 {
                    return Err(anyhow!(
                        "Graph has {node_count} nodes and CSR build failed: {e}. \
                         Run `sqry analyze` to precompute cycle data.",
                    ));
                }
                find_all_cycles_graph(core_cycle_type, &graph, &config)
...
pub fn execute_find_unused(args: &FindUnusedArgs) -> Result<ToolExecution<FindUnusedData>> {
    ...
    let unused_symbols = if let Some((scc, cond)) =
        ...
        tracing::warn!(
            "Analysis files not found at {}. Run `sqry analyze` or `sqry index --with-analysis` to enable find_unused.",
...
pub fn execute_is_node_in_cycle(
    ...
    let target_node_id = snapshot.find_by_name(&args.symbol);

    let (in_cycle, cycle) = if let Some(node_id) = target_node_id {
        let core_cycle_type = match args.cycle_type {
            CycleType::Calls => CircularType::Calls,
            CycleType::Imports => CircularType::Imports,
            CycleType::Modules => CircularType::Modules,
...
/// Find a node by name with flexible matching.
///
/// Tries exact match first via `find_by_name()`, then falls back to suffix
/// matching on qualified names (e.g., searching for `build_all` matches
/// `GraphAnalyses::build_all`). This matches the behavior of `get_definition`.
///
/// Returns the first suffix match when multiple symbols share the same
/// unqualified name. This is acceptable for `direct_callers`/`direct_callees`
/// which operate on a single target node.
fn find_node_flexible(
    snapshot: &sqry_core::graph::unified::concurrent::GraphSnapshot,
    symbol: &str,
) -> Option<sqry_core::graph::unified::node::NodeId> {
    // Priority 1: Exact match via optimized interner lookup
    if let Some(node_id) = snapshot.find_by_name(symbol) {
        return Some(node_id);
    }

    // Priority 2: Suffix match on name or qualified name
    let strings = snapshot.strings();
    let suffix_with_sep_colon = format!("::{symbol}");
    let suffix_with_sep_dot = format!(".{symbol}");
    ...
    for (node_id, entry) in snapshot.iter_nodes() {
        ...
        if name == symbol
            ...
            || qualified_name.ends_with(&suffix_with_sep_colon)
            || qualified_name.ends_with(&suffix_with_sep_dot)
            || name.ends_with(&suffix_with_sep_colon)
            || name.ends_with(&suffix_with_sep_dot)
        {
            return Some(node_id);
        }
    }
...
pub fn execute_direct_callers(
    ...
    let target_node_id = find_node_flexible(&snapshot, &args.symbol).ok_or_else(|| {
        // Try to find similar symbols for a helpful error message
        let suggestions = find_similar_symbols(&snapshot, &args.symbol, 3);
        ...
        anyhow!(
            ...
            • Run 'semantic_search' to find the correct symbol name{}",
            args.symbol,
            suggestions_str
        )
    })?;
...
pub fn execute_direct_callees(
    ...
    let source_node_id = find_node_flexible(&snapshot, &args.symbol).ok_or_else(|| {
        // Try to find similar symbols for a helpful error message
        let suggestions = find_similar_symbols(&snapshot, &args.symbol, 3);
        let suggestions_str = if suggestions.is_empty() {
            String::new()
...
mod tests {
    ...
    fn create_test_graph_for_unused() -> CodeGraph {
        ...
        let _node_unused = graph.nodes_mut().alloc(unused_entry).unwrap();
    ...
    fn test_rust_entry_point_detection_main() {
        ...
        // Find main function
        let main_node = snapshot.nodes_by_symbol("main").into_iter().next().unwrap();
    ...
    fn test_rust_entry_point_detection_public_export() {
        ...
        let public_node = snapshot
            .nodes_by_symbol("public_function")
            .into_iter()
            .next()
            .unwrap();
    ...
    fn test_rust_entry_point_detection_private_not_entry() {
        ...
        let unused_node = snapshot
            .nodes_by_symbol("unused_helper")
            .into_iter()
            .next()
            .unwrap();
    ...
    fn test_entry_point_detection_integration() {
        ...
        let main_nodes: Vec<_> = snapshot.nodes_by_symbol("main");
        ...
        let public_nodes: Vec<_> = snapshot.nodes_by_symbol("public_function");
        ...
        let unused_nodes: Vec<_> = snapshot.nodes_by_symbol("unused_helper");
