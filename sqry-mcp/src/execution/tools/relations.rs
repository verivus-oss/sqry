...
fn find_nodes_by_name(snapshot: &GraphSnapshot, name: &str) -> Vec<NodeId> {
    ...
    // Fast path: indexed lookup by simple name
    if let Some(name_id) = strings.get(name) {
        let matches = indices.by_name(name_id);
        if !matches.is_empty() {
            return matches.to_vec();
        }
    }

    // Fall back: indexed lookup by qualified name
    // Try exact qualified name match first
    if let Some(qname_id) = strings.get(name) {
        let matches = indices.by_qualified_name(qname_id);
        if !matches.is_empty() {
            return matches.to_vec();
        }
    }

    // Last resort: suffix match on qualified names (e.g., "::symbol_name")
    // This is O(N) but only triggered when indexed lookups fail
