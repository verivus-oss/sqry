...
use crate::tools::params::{
    ...
    DirectCalleesParams, DirectCallersParams, DuplicateTypeParam, EdgeKindParam, ExplainCodeParams,
    ExportGraphParams, FindCyclesParams, FindDuplicatesParams, FindUnusedParams,
    GetDefinitionParams, GetDocumentSymbolsParams, GetGraphStatsParams, GetHoverInfoParams,
    GetIndexStatusParams, GetInsightsParams, GetReferencesParams, GetWorkspaceSymbolsParams,
    GraphFormatParam, HierarchicalSearchParams, IsNodeInCycleParams, ListFilesParams,
    ListSymbolsParams, PaginationParams, PatternSearchParams, RebuildIndexParams,
    RelationQueryParams, RelationTypeParam, SearchFiltersParams, SearchSimilarParams,
    SemanticDiffParams, SemanticSearchParams, ShowDependenciesParams, SqryAskParams,
...
impl SqryServer {
    ...
    /// Create a new `SqryServer` with custom timeout and retry delay.
    pub fn with_config(feature_flags: FeatureFlags, timeout_ms: u64, retry_delay_ms: u64) -> Self {
        let tool_router = Self::filtered_tool_router(&feature_flags);
        Self {
            feature_flags,
            timeout_ms,
        ...
        async move {
            ...
            match result {
                Ok(Ok(Ok(execution))) => {
                    ...
                    Self::build_response(execution)
                }
                Ok(Ok(Err(anyhow_err))) => {
                    Err(McpError::internal_error(anyhow_err.to_string(), None))
                }
