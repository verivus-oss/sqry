...
//
// semantic_search("main") → result contains entries with kind "function"
...
/// Scenario 1a: `semantic_search` for "main" returns at least one function-kind result.
#[test]
fn test_scenario1a_semantic_search_main_returns_functions() -> Result<()> {
    ...
    // The mini-workspace fixture or the real codebase must expose at least one
    // symbol named or described as a function.
    assert!(
...
fn test_scenario3b_complexity_metrics_returns_named_entries() -> Result<()> {
    ...
    let json = parse_json(&text)?;
    // Must have a data wrapper containing an array of complexity entries.
    let data = json
        .get("data")
    ...
    assert!(
        data.is_array(),
        "complexity_metrics data must be an array. Got: {data}"
    );
...
fn test_scenario7a_get_index_status_ready_with_nodes() -> Result<()> {
    ...
    // nodeCount > 0 (graph has at least one symbol).
    let node_count = data["nodeCount"].as_u64().unwrap_or(0);
    assert!(
        node_count > 0,
        "get_index_status should report nodeCount > 0. Got: {json}"
    );
