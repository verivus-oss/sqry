# sqry MCP Server - Troubleshooting Guide
...
1. **Check sqry CLI**:
   ```bash
   sqry --version
   # Should output: sqry 0.17.0 or later
   ```
...
3. **Test server directly**:
   ```bash
   echo '{"jsonrpc":"2.0","method":"tools/list","params":{},"id":1}' | /path/to/sqry-mcp
...
3. **Rebuild server**:
   ```bash
   cargo build --release -p sqry-mcp --force
   ```
...
2. **Verify index exists**:
   ```bash
   ls -la .sqry-index
   # Should see: .sqry-index and possibly .sqry-index.lock
   ```
...
```bash
echo '{"jsonrpc":"2.0","method":"tools/list","params":{},"id":1}' | /path/to/sqry-mcp | jq .
```
...
2. **Single workspace per server**
   - Use multiple server instances for multiple projects
   - Or change SQRY_MCP_WORKSPACE_ROOT
