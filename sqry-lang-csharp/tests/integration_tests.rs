...
"#;
    ...
    assert!(
        find_node_entry(&staging, "MyClass.PublicMethod", NodeKind::Method).is_some(),
        "PublicMethod method node not found"
    );
    assert!(
        find_node_entry(&staging, "MyClass.PrivateMethod", NodeKind::Method).is_some(),
...
"#;
    ...
    let get_data = find_node_entry(&staging, "AsyncService.GetDataAsync", NodeKind::Method)
    ...
    let process = find_node_entry(&staging, "AsyncService.ProcessAsync", NodeKind::Method)
...
"#;
    ...
    let exported_method = exports
        .iter()
        .any(|(_, target)| target.as_deref() == Some("Exported.PublicMethod"));
    let exported_property = exports
        .iter()
        .any(|(_, target)| target.as_deref() == Some("Exported.Count"));
