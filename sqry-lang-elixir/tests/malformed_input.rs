//! Malformed input tests for Elixir language plugin.
//!
...
fn test_deeply_nested_shallow() {
    let plugin = ElixirPlugin::default();
    let nested = MalformedInputBuilder::for_language("elixir").deeply_nested(depths::SHALLOW);
...
fn test_deeply_nested_medium() {
    let plugin = ElixirPlugin::default();
    let nested = MalformedInputBuilder::for_language("elixir").deeply_nested(depths::MEDIUM);
...
fn test_deeply_nested_deep() {
    let plugin = ElixirPlugin::default();
    let nested = MalformedInputBuilder::for_language("elixir").deeply_nested(depths::DEEP);
...
fn test_oversized_1mb() {
    let plugin = ElixirPlugin::default();
    let large = MalformedInputBuilder::for_language("elixir").oversized_1mb();

    // This is valid Elixir (repeated), so parse should succeed
...
fn test_oversized_10mb() {
    let plugin = ElixirPlugin::default();
    let large = MalformedInputBuilder::for_language("elixir").oversized_10mb();
...
fn test_random_bytes() {
    ...
    // Random bytes very unlikely to be valid Elixir, but tree-sitter is error-tolerant
    let _ = result;
}
