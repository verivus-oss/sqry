...
export async function activate(
  ...
  // Initialize the language client before proceeding
  try {
    await client.initialize();
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
    outputChannel.appendLine(`[sqry] Failed to initialize extension: ${message}`);

    // Show error to user with actionable options
    ...
    // Don't proceed with activation if initialization failed
    return;
  }
