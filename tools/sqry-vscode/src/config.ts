...
export async function resolveBinary(binary: string, fallbackPath?: string): Promise<string> {
  ...
  try {
    ...
    // Try fallback path (e.g., auto-downloaded binary) before throwing
    if (fallbackPath) {
      try {
        await fs.promises.access(fallbackPath, fs.constants.X_OK);
        return fallbackPath;
      } catch {
        // Fallback not executable, fall through to error
      }
    }
