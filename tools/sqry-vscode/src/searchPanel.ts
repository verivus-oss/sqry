...
class SqryDuplicatesItem extends vscode.TreeItem {
  constructor(groupCount: number, symbolCount: number) {
...
class SqryUnusedItem extends vscode.TreeItem {
  constructor(count: number) {
    ...
    this.description = count > 0 ? `${count} symbols` : "none";
  ...
  implements vscode.TreeDataProvider<vscode.TreeItem>
{
  ...
  private addCDPredicates(items: vscode.TreeItem[], status: SqryIndexStatus): void {
    ...
    const duplicatesCount = this.cachedDuplicatesResult?.total_groups ?? 0;
    const symbolsCount = this.cachedDuplicatesResult?.total_symbols ?? 0;
    items.push(new SqryDuplicatesItem(duplicatesCount, symbolsCount));

    const cyclesCount = this.cachedCircularResult?.total_cycles ?? 0;
    items.push(new SqryCircularItem(cyclesCount));

    const unusedCount = this.cachedUnusedResult?.total ?? 0;
    items.push(new SqryUnusedItem(unusedCount));
  }
  ...
  private addStatusIndicators(items: vscode.TreeItem[], status: SqryIndexStatus): void {
    if (status.age_seconds !== undefined) {
      items.push(new SqryStatItem(
        ...
        "history",
        status.created_at ? `Created: ${status.created_at}` : undefined,
      ));
    }
    ...
    if (status.stale) {
      items.push(new SqryStatItem(
        ...
        "Index may be stale",
        "warning",
        "Consider rebuilding the index with 'Sqry: Index Workspace'",
      ));
    }
