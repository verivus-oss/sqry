...
- Version-aligned with sqry `v4.9.0` (CLI/LSP/MCP).
...
1. **Install sqry CLI** (prerequisite):
   ...
   sqry --version  # Verify: 4.9.0+
   ```

2. **Install extension** from VSIX:
   ```bash
   code --install-extension sqry-vscode-4.9.0.vsix
   ```
...
- sqry CLI version 4.9.0 or later
...
# Produces: sqry-vscode-4.9.0.vsix
```
