...
### v4.10.1
- Version-aligned with sqry `v4.10.7` (CLI/LSP/MCP).
...
1. **Install sqry CLI** (prerequisite):
   ...
   sqry --version  # Verify: 4.10.7+
   ```

2. **Install extension** from VSIX:
   ```bash
   code --install-extension sqry-vscode-4.10.7.vsix
   ```
...
- sqry CLI version 4.10.7 or later
...
# Produces: sqry-vscode-4.10.7.vsix
```
