...
### v4.10.1
- Version-aligned with sqry `v4.10.5` (CLI/LSP/MCP).
...
1. **Install sqry CLI** (prerequisite):
   ...
   sqry --version  # Verify: 4.10.5+
   ```

2. **Install extension** from VSIX:
   ```bash
   code --install-extension sqry-vscode-4.10.5.vsix
   ```
...
- sqry CLI version 4.10.5 or later
...
# Produces: sqry-vscode-4.10.5.vsix
```
