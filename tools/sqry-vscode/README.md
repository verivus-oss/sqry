...
- **Pass 5 global linking**: Rust extern → C/C++ functions, JS/TS fetch → Python/Java/Go endpoints
- **HTTP route matching**: Automatic detection across 5 languages (JS, TS, Python, Java, Go)
- **FFI linking**: Rust extern "C" ↔ C/C++ function definitions

### OOP Edge Detection
- **16 languages** support Inherits/Implements edges
- **Rust**: `impl Trait for Type` with qualified paths
- **Go**: Struct/interface embedding
- **JavaScript/TypeScript**: Class inheritance, mixin patterns
- **Java**: Interface methods, Java 9+ private methods

### FFI Edge Detection
- **11 languages** support cross-language call detection
- **Rust**: `extern "C"` blocks with ABI-aware linking
- **Go**: CGo, syscall, `plugin.Open`
- **Java**: JNI, JNA, Panama Foreign Function API
- **JavaScript**: WebAssembly API, native addon detection
- **Python**: ctypes/cffi with false-positive prevention
...
1. **Install sqry CLI** (prerequisite):
   ...
   sqry --version  # Verify: 4.8.2+
   ```

2. **Install extension** from VSIX:
   ```bash
   code --install-extension sqry-vscode-4.8.2.vsix
   ```
...
- sqry CLI version 4.8.2 or later
...
# Produces: sqry-vscode-4.8.2.vsix
```
