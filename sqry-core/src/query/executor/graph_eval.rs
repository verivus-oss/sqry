...
fn match_name(
    ...
    let Some(name) = ctx.graph.strings().resolve(entry.name) else {
        return false;
    };
    ...
    match (operator, value) {
        ...
        (Operator::Equal, Value::String(expected)) => segments_match(&name, expected),
        (Operator::Regex, Value::Regex(regex_val)) => get_or_compile_regex(
            &regex_val.pattern,
            regex_val.flags.case_insensitive,
            regex_val.flags.multiline,
...
fn match_callers(ctx: &GraphEvalContext, node_id: NodeId, value: &Value) -> bool {
    ...
    for edge in ctx.graph.edges().edges_from(node_id) {
        if let EdgeKind::Calls { .. } = &edge.kind
            ...
            && let Some(callee_name) = ctx.graph.strings().resolve(target_entry.name)
        {
            // Use segments_match for qualified names
            if segments_match(&callee_name, target_name) {
                return true;
            }
            ...
            if let Some(method) = &method_part
                && let Some(callee_method) = extract_method_name(&callee_name)
...
fn match_callees(ctx: &GraphEvalContext, node_id: NodeId, value: &Value) -> bool {
    ...
    for edge in ctx.graph.edges().edges_to(node_id) {
        if let EdgeKind::Calls { .. } = &edge.kind
            ...
            && let Some(source_name) = ctx.graph.strings().resolve(source_entry.name)
            && segments_match(&source_name, caller_name)
        {
            return true;
        }
    }
...
fn match_imports(ctx: &GraphEvalContext, node_id: NodeId, value: &Value) -> bool {
    ...
    for (other_id, other_entry) in ctx.graph.nodes().iter() {
        ...
        if other_entry.kind == NodeKind::Import
            && let Some(module_name) = ctx.graph.strings().resolve(other_entry.name)
            && module_name.contains(target_module)
        {
            return true;
        }
...
fn import_edge_matches(graph: &CodeGraph, edge: &StoreEdgeRef, target_module: &str) -> bool {
    ...
    let target_name = graph
        .nodes()
        .get(edge.target)
        .and_then(|entry| graph.strings().resolve(entry.name));

    // Check alias
    let alias_match = alias
        .and_then(|sid| graph.strings().resolve(sid))
        .is_some_and(|alias_str| alias_str.contains(target_module));

    // Check wildcard
    let wildcard_match = *is_wildcard && target_module == "*";

    // SUBSTRING matching on target name
    target_name.is_some_and(|name| name.contains(target_module)) || alias_match || wildcard_match
}
...
fn precompute_importing_files(graph: &CodeGraph, target_module: &str) -> HashSet<FileId> {
    ...
    for (node_id, entry) in graph.nodes().iter() {
        if entry.kind == NodeKind::Import
            && let Some(module_name) = graph.strings().resolve(entry.name)
        {
            // Use CONTAINS for substring matching
            if module_name.contains(target_module) {
...
fn match_exports(ctx: &GraphEvalContext, node_id: NodeId, value: &Value) -> bool {
    ...
    // Get the node's name for matching
    let Some(name) = ctx.graph.strings().resolve(entry.name) else {
        return false;
    };

    // Check if name matches target
    if !segments_match(&name, target_name) {
        return false;
    }
...
fn match_references(
    ...
    let name_matches = match (operator, value) {
        (Operator::Equal, Value::String(target)) => {
            name.as_ref() == target || name.ends_with(&format!("::{target}"))
        }
        (Operator::Regex, Value::Regex(rv)) => get_or_compile_regex(
            &rv.pattern,
            rv.flags.case_insensitive,
...
fn match_implements(ctx: &GraphEvalContext, node_id: NodeId, value: &Value) -> bool {
    ...
    for edge in ctx.graph.edges().edges_from(node_id) {
        if let EdgeKind::Implements = &edge.kind
            && let Some(target_entry) = ctx.graph.nodes().get(edge.target)
            && let Some(name) = ctx.graph.strings().resolve(target_entry.name)
            && segments_match(&name, trait_name)
        {
            return true;
        }
    }
