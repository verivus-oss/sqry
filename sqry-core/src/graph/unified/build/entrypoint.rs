...
fn build_unified_graph_inner(
    ...
    tracker.start_phase(1, "Parsing and committing files", total_files);
    ...
    for chunk_range in chunks {
        ...
        let expected_edges = plan.total_edges as usize;
...
pub fn build_and_persist_graph_with_progress(
    ...
    use crate::graph::unified::analysis::persistence::persist_csr;
    use crate::graph::unified::analysis::{AnalysisIdentity, compute_node_id_hash};
    ...
    // Step 5: Build CSR adjacency (for dedup edge count only — no SCC/condensation)
    ...
    // Step 9: Construct AnalysisIdentity and persist CSR adjacency only
    ...
    progress.report(IndexProgress::SavingStarted {
        component_name: "CSR adjacency",
    });
    let csr_save_start = std::time::Instant::now();

    persist_csr(&adjacency, &identity, &storage.analysis_csr_path())
        .context("Failed to persist CSR adjacency")?;

    progress.report(IndexProgress::SavingCompleted {
        component_name: "CSR adjacency",
        save_duration: csr_save_start.elapsed(),
    });

    log::info!(
        "CSR adjacency persisted to {}",
        storage.analysis_dir().display()
    );
