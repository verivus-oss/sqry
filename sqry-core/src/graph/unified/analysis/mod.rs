...
pub use condensation::{CondensationDag, Interval};
...
impl GraphAnalyses {
    ...
    pub fn build_all(snapshot: &CompactionSnapshot) -> Result<Self> {
        ...
        // Step 3: Sequential condensation DAG + 2-hop labels
        let cond_calls = CondensationDag::build(&scc_calls, &adjacency)?;
        let cond_imports = CondensationDag::build(&scc_imports, &adjacency)?;
        let cond_references = CondensationDag::build(&scc_references, &adjacency)?;
        let cond_inherits = CondensationDag::build(&scc_inherits, &adjacency)?;
