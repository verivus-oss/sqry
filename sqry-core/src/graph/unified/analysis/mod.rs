...
pub use condensation::{BudgetExceededPolicy, CondensationDag, Interval, LabelBudgetConfig};
...
pub use persistence::{
    ...
    persist_condensation, persist_csr, persist_scc, try_load_scc, try_load_scc_and_condensation,
};
pub use scc::SccData;
...
impl GraphAnalyses {
    ...
    pub fn build_all_with_budget(
        ...
        // Step 2: Parallel SCC computation for all edge kinds
        // Create EdgeKind instances for filtering (metadata doesn't matter for SCC computation)
        let kind_calls = EdgeKind::Calls {
            argument_count: 0,
            is_async: false,
        };
        let kind_imports = EdgeKind::Imports {
            alias: None,
            is_wildcard: false,
        };
        let kind_references = EdgeKind::References;
        let kind_inherits = EdgeKind::Inherits;

        let (scc_calls_result, nested) = rayon::join(
            || SccData::compute_tarjan(&adjacency, &kind_calls),
            || {
                rayon::join(
                    || SccData::compute_tarjan(&adjacency, &kind_imports),
                    || {
                        rayon::join(
                            || SccData::compute_tarjan(&adjacency, &kind_references),
                            || SccData::compute_tarjan(&adjacency, &kind_inherits),
                        )
                    },
                )
            },
        ...
        let scc_calls = scc_calls_result?;
        let (scc_imports_result, nested2) = nested;
        let scc_imports = scc_imports_result?;
        let (scc_references_result, scc_inherits_result) = nested2;
        let scc_references = scc_references_result?;
        let scc_inherits = scc_inherits_result?;

        // Step 3: Parallel condensation DAG + 2-hop labels
        let (cond_calls_result, nested_cond) = rayon::join(
            || CondensationDag::build_with_budget(&scc_calls, &adjacency, label_budget),
            || {
                rayon::join(
                    || CondensationDag::build_with_budget(&scc_imports, &adjacency, label_budget),
                    || {
                        rayon::join(
                            || {
                                CondensationDag::build_with_budget(
                                    &scc_references,
                                    &adjacency,
                                    label_budget,
                                )
        ...
        let cond_calls = cond_calls_result?;
        let (cond_imports_result, nested_cond2) = nested_cond;
        let cond_imports = cond_imports_result?;
        let (cond_references_result, cond_inherits_result) = nested_cond2;
        let cond_references = cond_references_result?;
        let cond_inherits = cond_inherits_result?;
