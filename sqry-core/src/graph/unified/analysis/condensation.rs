...
impl CondensationDag {
    ...
    ///
    /// When interval labels are available (`IntervalLabels` strategy), uses O(|L_out| + |L_in|)
        ...
        while let Some(current) = queue.pop_front() {
            ...
            for &neighbor in self.successors(current) {
                if !visited.contains(&neighbor) {
                    // When labels are available, prune unreachable neighbors
                    let should_explore = match self.strategy {
                        ReachabilityStrategy::IntervalLabels => {
                            self.can_reach_via_labels(neighbor, to_scc)
                        }
