...
//!
//! # Design (FR-53, FR-57, FR-63)
//!
//! - **Reservation-based**: Writers must acquire a reservation before writing
//! - **Dual CAS loops**: Atomic reservation of both bytes and ops (FR-57)
//! - **Compensating rollback**: CAS-based rollback prevents counter corruption (FR-63)
//!
//! # Thread Safety
//!
...
impl AdmissionController {
    ...
    /// rollback ensures no counter corruption (FR-63).
    ///
    /// # Arguments
    ///
    ...
    pub fn try_reserve(
        ...
        // Phase 1: CAS loop for bytes (FR-57)
        self.try_reserve_bytes(bytes)?;

        // Phase 2: CAS loop for ops (FR-57, with compensating rollback FR-63)
