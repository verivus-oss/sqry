...
impl ReservationGuard {
    ...
    pub fn commit_with_actual(
        ...
        // FR-64: Validate actual does not exceed reservation
        ...
        // FR-66: Release FULL reservation, commit only actual
        self.buffer_state
...
impl Drop for ReservationGuard {
    fn drop(&mut self) {
        ...
        if let Some(r) = self.reservation.take() {
            // FR-64: Full underflow protection in all builds (not just debug)
            let prev_bytes = self
                .buffer_state
                .reserved_bytes
