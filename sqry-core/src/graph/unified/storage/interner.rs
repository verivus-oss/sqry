...
mod tests {
    ...
    fn test_alloc_range_basic_succeeds() {
        // Verify that alloc_range returns the correct starting index and
        // advances the string count by the requested amount.  The capacity
        // boundary (LOCAL_TAG_BIT) cannot be exercised in a unit test because
        // allocating 2^31 slots would be prohibitive; that path is covered by
        // the error handling in alloc_range itself.
    ...
    fn test_with_max_ids_capacity_exhaustion_via_intern() {
        // with_max_ids limits the number of unique interned strings.
        let mut interner = StringInterner::with_max_ids(2);

        interner.intern("a").unwrap();
        interner.intern("b").unwrap();

        // Third unique string should hit the limit
        let err = interner.intern("c").unwrap_err();
        assert_eq!(err, InternError::CapacityExhausted);

        // Re-interning existing string (already in lookup) still works — no new slot
    ...
    fn test_with_max_ids_capacity_exhaustion_via_intern_without_ref() {
        ...
        // Second unique string should hit the limit
    ...
    fn test_intern_without_ref_already_interned_no_ref_count_change() {
        ...
        // intern_without_ref for an already-interned string returns the
        // existing ID but does NOT increment the ref count.
        let id2 = interner.intern_without_ref("hello").unwrap();
        assert_eq!(id, id2);
        assert_eq!(interner.ref_count(id), 1); // unchanged
    }

    #[test]
    fn test_recycle_unreferenced_noop_when_all_referenced() {
        let mut interner = StringInterner::new();
        interner.intern("a").unwrap(); // ref_count = 1
        interner.intern("b").unwrap(); // ref_count = 1

        let recycled = interner.recycle_unreferenced();
        assert_eq!(recycled, 0); // nothing to recycle
        assert_eq!(interner.len(), 2); // both strings still present
    }

    #[test]
    fn test_inc_ref_out_of_bounds_returns_none() {
        let mut interner = StringInterner::new();
        // index 999 is beyond the allocated range
    ...
    fn test_intern_without_ref_reuses_free_list_slot() {
        let mut interner = StringInterner::new();
        // Intern and then recycle to put a slot on the free list
        interner.intern_without_ref("recycle_me").unwrap(); // ref_count = 0
        interner.intern("anchor").unwrap(); // ref_count = 1, keeps interner non-empty
        let recycled = interner.recycle_unreferenced();
        assert_eq!(recycled, 1); // "recycle_me" freed

        // intern_without_ref for a new string should reuse the freed slot
