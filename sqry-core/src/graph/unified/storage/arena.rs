...
impl<T> Slot<T> {
    ...
    fn new_occupied(generation: u64, data: T) -> Self {
        Self {
            generation,
...
impl NodeArena {
    ...
    pub fn alloc(&mut self, entry: NodeEntry) -> Result<NodeId, ArenaError> {
        ...
        if let Some(free_idx) = self.free_head {
            ...
            // Increment generation for reuse detection (FR-48: respect MAX_GENERATION limit)
...
mod tests {
    ...
    fn test_generation_overflow_at_max_generation() {
        // FR-48: Test that allocation fails when generation reaches MAX_GENERATION
        let mut arena = NodeArena::new();
