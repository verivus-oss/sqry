...
//!
//! - [`CodeGraph`]: Arc-wrapped internals for O(1) `CoW` snapshots (FR-47)
//! - [`ConcurrentCodeGraph`]: `RwLock` wrapper with epoch versioning (FR-34)
//! - [`GraphSnapshot`]: Immutable snapshot for long-running queries (FR-35)
...
/// # Performance
///
/// - Snapshot creation: O(5) Arc clones ≈ <1μs (CP-5)
...
impl CodeGraph {
    ...
    ///
    /// This operation is O(5) Arc clones, which completes in <1μs (CP-5).
...
impl GraphSnapshot {
    ...
    /// Finds a node by qualified name.
    ///
    /// Searches for a node whose `qualified_name` field matches the given name.
    /// Returns the first matching node ID, or None if not found.
    ///
    /// # Performance
    ///
    /// Optimized to use `AuxiliaryIndices` for O(1) lookup.
    ///
    /// # Arguments
    ///
    /// * `name` - The qualified name to search for (e.g., "`module::Class::method`")
    ///
    /// # Returns
    ///
    /// The `NodeId` of the first matching node, or None if no match found.
    #[must_use]
    pub fn find_by_name(&self, name: &str) -> Option<crate::graph::unified::node::NodeId> {
        // Optimistic lookup: Check if the string exists in the interner
        let name_id = self.strings.get(name)?;

        // Priority 1: Exact match in qualified_name_index
        if let Some(node_id) = self.indices.by_qualified_name(name_id).first() {
            return Some(*node_id);
        }

        // Priority 2: Fallback to simple name match (as per legacy behavior)
        if let Some(node_id) = self.indices.by_name(name_id).first() {
            return Some(*node_id);
        }
    ...
    /// Finds all nodes with a given symbol name (not qualified).
    ///
    /// Searches for nodes whose simple name (not qualified name) matches
    /// the given symbol.
    ///
    /// # Arguments
    ///
    ...
    /// # Returns
    ///
    /// A vector of `NodeIds` for all nodes with this symbol name.
    #[must_use]
    pub fn nodes_by_symbol(&self, symbol: &str) -> Vec<crate::graph::unified::node::NodeId> {
        let mut matches = Vec::new();

        for (node_id, entry) in self.nodes.iter() {
            if let Some(name_str) = self.strings.resolve(entry.name)
                && name_str.as_ref() == symbol
            {
...
mod tests {
    ...
    fn test_code_graph_from_components() {
        ...
        let graph = CodeGraph::from_components(nodes, edges, strings, files, indices);
        assert_eq!(graph.epoch(), 0);
    }
    ...
    fn test_snapshot_find_by_name() {
        ...
        let found = snapshot.find_by_name("module::test_func");
        assert_eq!(found, Some(node_id));

        // Find by simple name (fallback)
        let found2 = snapshot.find_by_name("test_func");
    ...
    #[test]
    fn test_snapshot_nodes_by_symbol() {
        use crate::graph::unified::node::NodeKind;
        use crate::graph::unified::storage::arena::NodeEntry;
        use std::path::Path;
        ...
        // Find by symbol
        let matches = snapshot.nodes_by_symbol("test");
        assert_eq!(matches.len(), 2);
        assert!(matches.contains(&node1));
        assert!(matches.contains(&node2));

        // Find other symbol
        let matches = snapshot.nodes_by_symbol("other");
        assert_eq!(matches.len(), 1);
        assert_eq!(matches[0], node3);

        // No matches
        let matches = snapshot.nodes_by_symbol("nonexistent");
        assert!(matches.is_empty());
    }
    ...
    fn test_snapshot_query_empty_graph() {
        ...
        assert!(snapshot.find_by_name("test").is_none());
        assert!(snapshot.find_by_pattern("test").is_empty());
        assert!(snapshot.nodes_by_symbol("test").is_empty());
