...
mod tests {
    ...
    fn test_xxhash64_performance_characteristic() {
        ...
        // XXHash64 should hash 1MB in well under 10ms on any modern CPU
        // (typically <1ms, allowing generous margin for CI systems)
        assert!(
            elapsed.as_millis() < 20,
            "XXHash64 took {elapsed:?} to hash 1MB (expected <20ms)"
        );
    }
