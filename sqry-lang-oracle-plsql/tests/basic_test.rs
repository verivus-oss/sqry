...
fn collect_display_names(staging: &StagingGraph, language: Language) -> Vec<String> {
    staging
        .operations()
        .iter()
        .filter_map(|op| {
...
"#;
    ...
    let display_names = collect_display_names(&staging, Language::Plsql);
    assert!(display_names.iter().any(|name| name == "test_pkg.do_work"));
}
