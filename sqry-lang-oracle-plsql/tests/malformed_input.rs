//! Malformed input tests for Plsql language plugin.
//!
...
fn test_deeply_nested_shallow() {
    let plugin = OraclePlsqlPlugin::default();
    let nested = MalformedInputBuilder::for_language("plsql").deeply_nested(depths::SHALLOW);
...
fn test_deeply_nested_medium() {
    let plugin = OraclePlsqlPlugin::default();
    let nested = MalformedInputBuilder::for_language("plsql").deeply_nested(depths::MEDIUM);
...
fn test_deeply_nested_deep() {
    let plugin = OraclePlsqlPlugin::default();
    let nested = MalformedInputBuilder::for_language("plsql").deeply_nested(depths::DEEP);
...
fn test_oversized_1mb() {
    let plugin = OraclePlsqlPlugin::default();
    let large = MalformedInputBuilder::for_language("plsql").oversized_1mb();

    // This is valid Plsql (repeated), so parse should succeed
...
fn test_oversized_10mb() {
    let plugin = OraclePlsqlPlugin::default();
    let large = MalformedInputBuilder::for_language("plsql").oversized_10mb();
...
fn test_random_bytes() {
    ...
    // Random bytes very unlikely to be valid Plsql, but tree-sitter is error-tolerant
    let _ = result;
}
