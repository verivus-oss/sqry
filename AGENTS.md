...
- **Variable-level rules**: See `docs/agent-process/VARIABLE_NAMING_CONVENTIONS.md` for booleans, collections, handles, and AST-specific naming guidance; record any exception in the component’s planning docs.
...
- **Naming conventions** (`docs/agent-process/VARIABLE_NAMING_CONVENTIONS.md`): Project-wide patterns (e.g., `temp_dir` → `tmp_*`, booleans → `is_/has_/should_*`)
...
| crgrep port | PORT_PLAN + retrospective docs after import | Code review mandatory; planning review if new behavior added | Explain deviations from original logic |
...
### Core Rule
**No code without specification** (unless porting from crgrep - see below).
...
- `PORT_TEMPLATE.md` for crgrep ports
...
- See `docs/agent-process/GEMINI_WRITE_FIX.md` for complete troubleshooting guide
...
Based on the request, this is a: [major component / plugin / port from crgrep]

Required process:
- Major component: Full 6-document process (see DEVELOPMENT_PROCESS.md)
- Plugin: Simplified 3-document process
- Port from crgrep: Code-first + retrospective docs
