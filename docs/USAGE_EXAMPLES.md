...
### Comparing Refs Semantically

```bash
# Compare two branches at symbol level
sqry diff main feature/refactor-auth

# Focus on API-impacting changes
sqry diff v2.3.0 v2.4.0 --change-type signature_changed --kind function,method

# Find new symbols only (good for changelog generation)
sqry diff release/2026.02 release/2026.03 --change-type added --json
```
