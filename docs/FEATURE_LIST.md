...
| ~~`sqry diff`~~ | Not yet implemented — hidden from CLI help (backlog item A6) |
...
The VS Code extension (`tools/sqry-vscode`) builds on `sqry lsp` and exposes:
...
- `NodeKind` - 22 symbol kinds (Function, Class, Method, etc.)
- `EdgeKind` - 20+ edge types with metadata (Calls, Imports, Inherits, etc.)
...
## Supported Languages (34 plugins)
...
sqry supports a powerful query syntax with multiple predicates, operators, and combinators.
