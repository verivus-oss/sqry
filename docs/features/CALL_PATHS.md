...
### 5. Linux Benchmark Observations

From Linux-kernel scale validation (`docs/launch/TECHNICAL_ARTICLE.md`), path
tracing is effective but has explicit boundaries:

- Macro-generated entry points (for example `SYSCALL_DEFINE*`) can obscure
  symbol identity and break static continuity.
- Function-pointer dispatch (for example `file->f_op->read_iter`) often marks
  trace boundaries where static targets are not fully recoverable.
- Mixed configuration branches (`#ifdef CONFIG_*`) may yield source-space paths
  that are mutually exclusive in a concrete build.
- Assembly and loadable-module boundaries can produce partial paths.

Operational guidance:
- Treat a full path as strong static evidence, not runtime proof.
- Treat a partial path as a narrowed investigation region.
- Pair high-impact traces with runtime validation (`ftrace`, `perf`, BPF).
