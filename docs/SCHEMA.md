# sqry Data Dictionary

This document defines the canonical types used across all sqry interfaces (CLI, LSP, MCP).
All types are defined in `sqry-core/src/schema/` and should be the single source of truth.
...
- `is_container()` → `true` for Class, Interface, Trait, Struct, Module, Enum
...
| `type_of` | - | Type reference (return type, parameter type) |
