...
//!
//! **Source Grammar**: <https://github.com/murtaza64/tree-sitter-groovy>
...
/// # Safety
///
/// This function wraps an `unsafe` FFI call to `tree_sitter_groovy()`.
...
pub fn language() -> Language {
    // SAFETY: tree_sitter_groovy() is an extern C function from the linked
    // tree-sitter-groovy library. We immediately validate the returned Language
    ...
    let lang = unsafe { tree_sitter_groovy() };
    sqry_tree_sitter_support::validate_language_or_panic(lang, "Groovy")
}
...
pub const NODE_TYPES: &str = include_str!("../../../vendor/tree-sitter-groovy/src/node-types.json");
