//! Malformed input tests for Apex language plugin.
//!
...
fn test_deeply_nested_shallow() {
    let plugin = SalesforceApexPlugin::default();
    let nested = MalformedInputBuilder::for_language("apex").deeply_nested(depths::SHALLOW);
...
fn test_deeply_nested_medium() {
    let plugin = SalesforceApexPlugin::default();
    let nested = MalformedInputBuilder::for_language("apex").deeply_nested(depths::MEDIUM);
...
fn test_deeply_nested_deep() {
    let plugin = SalesforceApexPlugin::default();
    let nested = MalformedInputBuilder::for_language("apex").deeply_nested(depths::DEEP);
...
fn test_oversized_1mb() {
    let plugin = SalesforceApexPlugin::default();
    let large = MalformedInputBuilder::for_language("apex").oversized_1mb();

    // This is valid Apex (repeated), so parse should succeed
...
fn test_oversized_10mb() {
    let plugin = SalesforceApexPlugin::default();
    let large = MalformedInputBuilder::for_language("apex").oversized_10mb();
...
fn test_random_bytes() {
    ...
    // Random bytes very unlikely to be valid Apex, but tree-sitter is error-tolerant
    let _ = result;
}
