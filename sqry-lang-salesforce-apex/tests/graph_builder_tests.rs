...
fn build_node_canonical_lookup(staging: &StagingGraph) -> HashMap<NodeId, (String, NodeKind)> {
    ...
    for op in staging.operations() {
        ...
        } = op
        {
            ...
            let name = strings.get(&name_id.index()).cloned().unwrap_or_default();
            nodes.insert(*node_id, (name, entry.kind));
        }
    }
    nodes
...
fn build_node_display_lookup(staging: &StagingGraph) -> HashMap<NodeId, (String, NodeKind)> {
    ...
    for op in staging.operations() {
        ...
        {
            let name = staging
                .resolve_node_display_name(Language::Apex, entry)
                .unwrap_or_default()
                .to_string();
            nodes.insert(*node_id, (name, entry.kind));
        }
...
fn has_annotation_edge(staging: &StagingGraph, method_name: &str, annotation: &str) -> bool {
    let source_nodes = build_node_lookup(staging);
    let target_nodes = build_node_canonical_lookup(staging);
    ...
    for op in staging.operations() {
        ...
        } = op
        {
            ...
            let source_name = source_nodes.get(source).map(|(name, _)| name.as_str());
            let target_name = target_nodes.get(target).map(|(name, _)| name.as_str());
...
fn collect_typeof_edges(staging: &StagingGraph) -> Vec<(String, String)> {
    ...
    staging
        ...
        .filter_map(|op| {
            ...
            {
                let from_name = source_nodes.get(source).map(|(name, _)| name.clone());
                let to_name = target_nodes.get(target).map(|(name, _)| name.clone());
...
fn collect_typeof_edges_for_display(staging: &StagingGraph) -> Vec<(String, String)> {
    let source_nodes = build_node_lookup(staging);
    let target_nodes = build_node_display_lookup(staging);
    staging
        .operations()
        .iter()
        .filter_map(|op| {
...
";
    ...
    let edges = collect_typeof_edges(&staging);
    assert!(
        edges.iter().any(|(_, to)| to == "Outer::Inner"),
        "Expected canonical TypeOf edge with scoped type, got: {edges:?}"
    );

    let display_edges = collect_typeof_edges_for_display(&staging);
    assert!(
        display_edges.iter().any(|(_, to)| to == "Outer.Inner"),
        "Expected Apex display TypeOf edge with scoped type, got: {display_edges:?}"
    );
}
