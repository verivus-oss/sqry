//! Malformed input tests for Ruby language plugin.
//!
...
fn test_deeply_nested_shallow() {
    let plugin = RubyPlugin::default();
    let nested = MalformedInputBuilder::for_language("ruby").deeply_nested(depths::SHALLOW);
...
fn test_deeply_nested_medium() {
    let plugin = RubyPlugin::default();
    let nested = MalformedInputBuilder::for_language("ruby").deeply_nested(depths::MEDIUM);
...
fn test_deeply_nested_deep() {
    let plugin = RubyPlugin::default();
    let nested = MalformedInputBuilder::for_language("ruby").deeply_nested(depths::DEEP);
...
fn test_oversized_1mb() {
    let plugin = RubyPlugin::default();
    let large = MalformedInputBuilder::for_language("ruby").oversized_1mb();

    // This is valid Ruby (repeated), so parse should succeed
...
fn test_oversized_10mb() {
    let plugin = RubyPlugin::default();
    let large = MalformedInputBuilder::for_language("ruby").oversized_10mb();
...
fn test_random_bytes() {
    ...
    // Random bytes very unlikely to be valid Ruby, but tree-sitter is error-tolerant
    let _ = result;
}
