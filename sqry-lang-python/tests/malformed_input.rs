//! Malformed input tests for Python language plugin.
//!
...
fn test_deeply_nested_shallow() {
    let plugin = PythonPlugin::default();
    let nested = MalformedInputBuilder::for_language("python").deeply_nested(depths::SHALLOW);
...
fn test_deeply_nested_medium() {
    let plugin = PythonPlugin::default();
    let nested = MalformedInputBuilder::for_language("python").deeply_nested(depths::MEDIUM);
...
fn test_deeply_nested_deep() {
    let plugin = PythonPlugin::default();
    let nested = MalformedInputBuilder::for_language("python").deeply_nested(depths::DEEP);
...
fn test_oversized_1mb() {
    ...
    // This is valid Python (repeated), so parse should succeed
...
fn test_oversized_10mb() {
    let plugin = PythonPlugin::default();
    let large = MalformedInputBuilder::for_language("python").oversized_10mb();
...
fn test_random_bytes() {
    ...
    // Random bytes very unlikely to be valid Python, but tree-sitter is error-tolerant
    let _ = result;
}
