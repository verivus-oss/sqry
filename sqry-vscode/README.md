...
- **Analysis Panels**: Duplicate code, circular dependencies, unused symbols (lazy-loaded with "expand to check" placeholders)
...
- **Real-time Progress**: Detailed progress during indexing with auto-refresh of stats on completion
...
- **Auto-refresh stats**: Index stats panel refreshes automatically after rebuild completes
- **Debug diagnostics**: LSP logs silent node conversion failures for easier troubleshooting

### v4.10.1
- Version-aligned with sqry `v4.10.13` (CLI/LSP/MCP).
...
1. **Install sqry CLI** (prerequisite):
   ...
   sqry --version  # Verify: 4.10.13+
   ```

2. **Install extension** from VSIX:
   ```bash
   code --install-extension sqry-vscode-4.10.13.vsix
   ```
...
- sqry CLI version 4.10.13 or later
...
# Produces: sqry-vscode-4.10.13.vsix
```
