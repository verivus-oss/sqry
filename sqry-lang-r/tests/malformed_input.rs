//! Malformed input tests for R language plugin.
//!
...
fn test_deeply_nested_shallow() {
    let plugin = RPlugin::default();
    let nested = MalformedInputBuilder::for_language("r").deeply_nested(depths::SHALLOW);
...
fn test_deeply_nested_medium() {
    let plugin = RPlugin::default();
    let nested = MalformedInputBuilder::for_language("r").deeply_nested(depths::MEDIUM);
...
fn test_deeply_nested_deep() {
    let plugin = RPlugin::default();
    let nested = MalformedInputBuilder::for_language("r").deeply_nested(depths::DEEP);
...
fn test_oversized_1mb() {
    let plugin = RPlugin::default();
    let large = MalformedInputBuilder::for_language("r").oversized_1mb();

    // This is valid R (repeated), so parse should succeed
...
fn test_oversized_10mb() {
    let plugin = RPlugin::default();
    let large = MalformedInputBuilder::for_language("r").oversized_10mb();
...
fn test_random_bytes() {
    ...
    // Random bytes very unlikely to be valid R, but tree-sitter is error-tolerant
    let _ = result;
}
