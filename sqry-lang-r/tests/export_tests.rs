...
fn build_node_lookup(staging: &StagingGraph) -> HashMap<NodeId, (String, String, NodeKind)> {
    let _strings = build_string_lookup(staging);
    ...
    for op in staging.operations() {
        ...
        } = op
        {
            ...
            let display_name = staging
                .resolve_node_display_name(Language::R, entry)
                .unwrap_or_else(|| canonical_name.clone());
            nodes.insert(*node_id, (canonical_name, display_name, entry.kind));
        }
    }
    nodes
}
    ...
    for op in staging.operations() {
        ...
        } = op
        {
            let target_name = nodes
                .get(target)
                .map(|(canonical_name, _, _)| canonical_name.as_str());
...
";
    ...
    assert!(
        ...
        "Expected canonical export edge for S3 method print::myclass"
    );
    assert!(
        has_display_export_edge(&staging, "print.myclass"),
        "Expected display export edge for S3 method print.myclass"
    );
    assert!(
