...
fn staged_nodes(source: &str) -> Vec<(String, u32, u32)> {
    ...
    sqry_lang_shell::relations::ShellGraphBuilder::default()
        .build_graph(&tree, source.as_bytes(), Path::new("q.sh"), &mut staging)
        .expect("build graph");
...
/// Function mints alongside the whole-file module node.
#[test]
fn function_mints_keep_columns_inside_their_lines() {
