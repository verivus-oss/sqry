//! Malformed input tests for C language plugin.
//!
...
fn test_deeply_nested_shallow() {
    let plugin = CPlugin::default();
    let nested = MalformedInputBuilder::for_language("c").deeply_nested(depths::SHALLOW);
...
fn test_deeply_nested_medium() {
    let plugin = CPlugin::default();
    let nested = MalformedInputBuilder::for_language("c").deeply_nested(depths::MEDIUM);
...
fn test_deeply_nested_deep() {
    let plugin = CPlugin::default();
    let nested = MalformedInputBuilder::for_language("c").deeply_nested(depths::DEEP);
...
fn test_oversized_1mb() {
    let plugin = CPlugin::default();
    let large = MalformedInputBuilder::for_language("c").oversized_1mb();

    // This is valid C (repeated), so parse should succeed
...
fn test_oversized_10mb() {
    let plugin = CPlugin::default();
    let large = MalformedInputBuilder::for_language("c").oversized_10mb();
...
fn test_random_bytes() {
    ...
    // Random bytes very unlikely to be valid C, but tree-sitter is error-tolerant
    let _ = result;
}
