...
fn test_struct_field_calls() {
    ...
    // Build a map from NodeId -> name for resolving edge endpoints
    let node_names: std::collections::HashMap<_, _> = staging
        .nodes()
        .filter_map(|node_ref| {
            let name = staging.resolve_node_name(node_ref.entry)?;
            Some((node_ref.expected_id?, name.to_string()))
        })
        .collect();

    // Verify my_fops variable exists
    let my_fops_id = node_names
        .iter()
        .find(|(_, name)| *name == "my_fops")
        .map(|(id, _)| *id)
        .expect("my_fops variable should be staged");

    // Collect References edges originating from my_fops
    let ref_targets: std::collections::HashSet<String> = staging
        .edges()
        .filter(|edge| edge.source == my_fops_id && matches!(edge.kind, EdgeKind::References))
        .filter_map(|edge| node_names.get(&edge.target).cloned())
        .collect();

    // Verify designated initializer targets are present (there may be additional
    // References edges from type resolution, e.g., to "struct file_operations")
    for target in ["my_read", "my_write", "my_open"] {
        assert!(
            ref_targets.contains(target),
            "my_fops should reference {target}, got: {ref_targets:?}"
        );
    }
}

#[test]
fn test_designated_initializer_multi_variable_attribution() {
    // Verifies that multi-variable declarations like:
    //   struct ops fops_a = { .read = my_read }, fops_b = { .read = alt_read };
    // correctly attribute each initializer's targets to the right variable.
    let content = include_str!("fixtures/c/struct_field_calls.c");
    let tree = parse_c_file(content);
    let mut staging = StagingGraph::new();
    let builder = CGraphBuilder::default();
    ...
    // Helper: collect References targets for a given variable name
    let refs_for = |var_name: &str| -> std::collections::HashSet<String> {
        let var_id = node_names
            .iter()
            .find(|(_, name)| *name == var_name)
            .map(|(id, _)| *id)
            .unwrap_or_else(|| panic!("{var_name} should be staged"));
        staging
            .edges()
            .filter(|e| e.source == var_id && matches!(e.kind, EdgeKind::References))
            .filter_map(|e| node_names.get(&e.target).cloned())
            .collect()
    };

    let fops_a_refs = refs_for("fops_a");
    let fops_b_refs = refs_for("fops_b");

    // fops_a should reference my_read and my_write (NOT alt_read/alt_write)
    assert!(
        fops_a_refs.contains("my_read"),
        "fops_a should reference my_read, got: {fops_a_refs:?}"
    );
    assert!(
        fops_a_refs.contains("my_write"),
        "fops_a should reference my_write, got: {fops_a_refs:?}"
    );
    assert!(
        !fops_a_refs.contains("alt_read"),
        "fops_a should NOT reference alt_read (belongs to fops_b), got: {fops_a_refs:?}"
    );

    // fops_b should reference alt_read and alt_write (NOT my_read/my_write)
    assert!(
        fops_b_refs.contains("alt_read"),
        "fops_b should reference alt_read, got: {fops_b_refs:?}"
    );
    assert!(
        fops_b_refs.contains("alt_write"),
        "fops_b should reference alt_write, got: {fops_b_refs:?}"
    );
    assert!(
        !fops_b_refs.contains("my_read"),
        "fops_b should NOT reference my_read (belongs to fops_a), got: {fops_b_refs:?}"
    );
}
