//! Daemon IPC wrapper for the `find_cycles` MCP tool method.

use serde_json::Value;

use sqry_mcp::daemon_adapter::execute_find_cycles_for_daemon;
use sqry_mcp::daemon_params::params_to_find_cycles_args;
...
pub(crate) async fn handle(ctx: &HandlerContext, params: Value) -> Result<Value, MethodError> {
    ...
    classify_and_build(ctx, "find_cycles", &path, move |wctx| {
        execute_find_cycles_for_daemon(wctx, &args)
    })
    .await
}
