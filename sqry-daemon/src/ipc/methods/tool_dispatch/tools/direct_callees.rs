//! Daemon IPC wrapper for the `direct_callees` MCP tool method.

use serde_json::Value;

use sqry_mcp::daemon_adapter::execute_direct_callees_for_daemon;
use sqry_mcp::daemon_params::params_to_direct_callees_args;
...
pub(crate) async fn handle(ctx: &HandlerContext, params: Value) -> Result<Value, MethodError> {
    ...
    classify_and_build(ctx, "direct_callees", &path, move |wctx, _cancel| {
        execute_direct_callees_for_daemon(wctx, &args, std::time::Instant::now())
    })
    .await
}
