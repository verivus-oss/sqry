//! Malformed input tests for Puppet language plugin.
//!
...
fn test_deeply_nested_shallow() {
    let plugin = PuppetPlugin::default();
    let nested = MalformedInputBuilder::for_language("puppet").deeply_nested(depths::SHALLOW);
...
fn test_deeply_nested_medium() {
    let plugin = PuppetPlugin::default();
    let nested = MalformedInputBuilder::for_language("puppet").deeply_nested(depths::MEDIUM);
...
fn test_deeply_nested_deep() {
    let plugin = PuppetPlugin::default();
    let nested = MalformedInputBuilder::for_language("puppet").deeply_nested(depths::DEEP);
...
fn test_oversized_1mb() {
    let plugin = PuppetPlugin::default();
    let large = MalformedInputBuilder::for_language("puppet").oversized_1mb();

    // This is valid Puppet (repeated), so parse should succeed
...
fn test_oversized_10mb() {
    let plugin = PuppetPlugin::default();
    let large = MalformedInputBuilder::for_language("puppet").oversized_10mb();
...
fn test_random_bytes() {
    ...
    // Random bytes very unlikely to be valid Puppet, but tree-sitter is error-tolerant
    let _ = result;
}
