//! Malformed input tests for Swift language plugin.
//!
...
use sqry_tree_sitter_fuzz_support::testing::run_with_stack;
use std::path::Path;
...
fn test_deeply_nested_shallow() {
    let plugin = SwiftPlugin::default();
    let nested = MalformedInputBuilder::for_language("swift").deeply_nested(depths::SHALLOW);
...
fn test_deeply_nested_medium() {
    let plugin = SwiftPlugin::default();
    let nested = MalformedInputBuilder::for_language("swift").deeply_nested(depths::MEDIUM);
...
fn test_deeply_nested_deep() {
    let plugin = SwiftPlugin::default();
    let nested = MalformedInputBuilder::for_language("swift").deeply_nested(depths::DEEP);
...
fn test_oversized_1mb() {
    let plugin = SwiftPlugin::default();
    let large = MalformedInputBuilder::for_language("swift").oversized_1mb();

    // This is valid Swift (repeated), so parse should succeed
...
fn test_oversized_10mb() {
    let plugin = SwiftPlugin::default();
    let large = MalformedInputBuilder::for_language("swift").oversized_10mb();
...
fn test_random_bytes() {
    ...
    // Random bytes very unlikely to be valid Swift, but tree-sitter is error-tolerant
    let _ = result;
}
